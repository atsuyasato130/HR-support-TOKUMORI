...
**起動**:
```bash
cd "/Users/atsuyasato130/Claude AI/AI agent（HRsupport事業）/business/career_advisor"
python3 main.py
```
...
## 0.55 グループDM(mpim)タスクハンドラー（2026-07-02 追加）

**背景**: Sato/Haruka Mukai の1:1 Slack DMにSlack純正「AI Assistant」アプリを追加してグループDM(mpim)化。hr-support Botも同じmpimに招待し、Sato以外(Haruka)からもタスク依頼をSlack経由で処理できるようにする。

**実装**: `communication/slack_bot.py` に既存の `on_mention()` / `on_dm()` と並列で `on_group_dm_task()`（`message.mpim` 購読）を追加。既存ハンドラーは無改修。

```
グループDMメッセージ受信
  ↓ channel_type == "mpim" かつ domain_hr.json.group_dm_task_handler.allowed_channel_ids に一致？
  ↓（不一致・disabled なら無反応）
_classify_group_dm_intent()
  ├─ 定型パターン一致(紹介文/SF選考登録) → _dispatch_known_task_or_none()（既存の安全な定型ロジックへ）
  ├─ タスクっぽいキーワードなし        → 無反応(雑談)
  └─ キーワードありだが定型外          → Claude Haikuで最終分類 → 汎用 run_agent()
       （分類API失敗時は chit_chat と区別し "🤖 うまく判定できませんでした" を返す）
```

**安全設計（security-reviewerレビュー反映）**:
- チャンネルIDホワイトリスト方式（誤招待・他mpimでの誤爆防止）
- 汎用応答(`general_task`)は縮小ツールセット `GROUP_DM_TOOLS` のみ使用可能。**SF更新/新規作成・メール送信・LINE送信・任意チャンネルへのSlack投稿は除外**（Sheets読み書き・Calendar・Docs・Notion・SF/Slack読み取りのみ許可）。SF選考登録は `_dispatch_known_task_or_none()` 経由の定型パターンでのみ実行可能
- グループDM経由の`run_agent()`は`thread_id=f"mpim_{channel}"`で1:1 DMの会話履歴と分離
- エラー発生時は内部例外の詳細を複数人のいるDMに露出させず、`error_message`引数で定型文に差し替え（ログには詳細を残す）
- 事前承認フロー（実行前の人間確認）は今回未実装。運用して誤操作が問題化した場合に追加検討する拡張ポイント

**設定**: `config/domain_hr.json` の `group_dm_task_handler`（`enabled` / `allowed_channel_ids`）。`utils/domain_config.py` の `get_group_dm_task_config()` で読み取り。

**登録**: `knowledge/hr_support/AGENT_MANIFEST.json` の `hr_executor_group_dm_task`
...
### v2 大規模再構築 (2026-04-23)

**アーキテクチャ3層分離**:
- DB層: `Sales_Development__c`（Label: アプローチ対象企業、企業マスターに純化） + 9子オブジェクト + `Call_Log__c`（架電ログ、1架電1レコード）
- UI層: SNS就活（NewGraduates）アプリ内に **2タブ追加**
  - Tab A: アプローチ対象企業（SF標準タブ、求人票同様のUI、リッチRecordPageでKIJI再現）
  - Tab B: Sales Development（`sdDashboard` LWC、7タブ内蔵型ダッシュボード）
- 架電記録: Tab A RecordPage の Quick Actions ボタンから Call_Log__c 作成

**子オブジェクト9種**:
SD_Department (部署情報) / SD_Officer (役員) / SD_Job_Posting (求人) / SD_News (ニュース) /
SD_Employee_History (従業員数推移) / SD_Business_Line (事業概要) / SD_Personnel_Change (人事異動) /
SD_Location (拠点) / SD_Financial (財務推移)

**LWC 16種**（RecordPage + Dashboard）:
- 既存: sdAdvancedSearch / sdKanban / sdQuickActions (Call_Log対応に改修)
- 新規: sdCompanyHeader / sdDepartmentList / sdNewsFeed / sdJobPostings /
       sdBusinessOverview / sdOfficerList / sdPersonnelChanges / sdEmployeeTrend /
       sdApproachScore / sdUnifiedTimeline / sdSimilarCompanies / sdAiSummaryButton /
       sdDashboard (Tab B メイン、7タブ: 企業検索/架電ログ/Kanban/担当者KPI/掘り起こし/一括メール/商談引継ぎ)

**Apex Controller `SalesDevelopmentController`（17メソッド）**:
- search / createCallLog / logCall / logConnected / logAppointment / logUnavailable / logLost
- getCallLogs / getCallLogsFiltered / getSummary / getConsultantKpis
- setReapproach / getReapproachList / getSimilarCompanies / getUnifiedTimeline
- getByStatus / updateStatus (Kanban用、Call_Log作成で履歴保持) / bulkSendEmail

**独自機能（KIJI超え）**:
- アプローチ推奨度メーター（Lead_Score可視化+根拠）
- 統合タイムライン（架電・ニュース・求人・人事異動を1本の時系列）
- 類似企業サジェスト（業種×規模×タグ）
- AI営業サマリー生成（Phase 2でClaude API実連携予定）
- 一括メールアプローチ（Tab Bダッシュボード）
- 掘り起こしリスト（Reapproach_Date__c × 期間絞込）

**追加フィールド**:
- Sales_Development__c: Converted_Account/Pipeline/Date（FS連携）、AI_Sales_Summary、
  Reapproach_Date/Memo（掘り起こし）、Legal_Number、Postal_Code、Job_Categories
- Call_Log__c: Channel（電話/メール/SMS/対面）、Email_Subject/Body/Opened/Replied

### v3 改修 (2026-04-24)

**変更点**:
- Kanbanタブ削除 → `sdTodayList`（今日のToDo: 未着手+再架電+掘り起こし統合）
- 新オブジェクト `Sales_Target__c`（日次/週次/月次 KPI目標、個人/チーム単位）
- Sales_Development__c に THE MODEL型フィールド: `Lead_Source__c`, `SDR_Stage__c`, `Assigned_To__c`, `Assigned_At__c`
- Call_Log__c に `CallLogTrigger` → アポ獲得時 `SlackNotifier` で通知（Webhook URL未設定時はスキップ）
- 新LWC: `sdTodayList`, `sdTargetDashboard`, `sdLeaderboard`
- sdDashboard タブ構成: 📋今日のToDo / 🔍企業検索(インラインアクション付) / 📞架電ログ / 👥担当者KPI(目標vs実績) / 🏆リーダーボード / 🎯掘り起こし / ✉️一括メール / 🤝商談引継ぎ
- Apex拡張メソッド: `getTodayTodo`, `getTargetAchievement`, `createAppointment`(Event自動作成), `convertToFS`(THE MODEL), `getLeaderboard`(バッジシステム), `assignToMe`/`releaseAssignment`(同時編集), `transferOwnership`(担当者引継ぎ)

### v4 改修 (2026-04-24)

**UX改善**:
- 企業検索のインラインアクション（6ボタン）→ 1プルダウンに集約
- 👁 サマリーボタン → 新モーダル `sdCompanyQuickView`（企業6セクション一括表示）

**詳細分析タブ追加**: 新LWC `sdAnalytics`（6サブタブ内蔵）
- ⏰ 曜日×時間帯接続率ヒートマップ
- 🎯 セグメント別アポ率（業種/都道府県/流入経路）
- 📈 個人12週トレンド（架電/接続/アポ折れ線）
- 🔄 ファネル分析（対象→架電→接続→アポ→FS引継ぎ）
- ❌ 失注理由分析
- 💡 1on1サマリー（先月vs今月比較+推奨アクション）

**新Apexメソッド**: getCompanyQuickSummary, getHeatmap, getSegmentAnalysis, getUserTrend, getFunnelAnalysis, getLossAnalysis, get1on1Summary

**Tab B タブ構成**: 9タブに拡張（📋今日のToDo / 🔍企業検索 / 📞架電ログ / 👥担当者KPI / 🏆リーダーボード / 📊詳細分析 / 🎯掘り起こし / ✉️一括メール / 🤝商談引継ぎ）

### 次フェーズ（未実装）
- Phase 2: Cloud Run + gBizINFO/年金機構/マイナビ/PRTimes/firecrawl スクレイピング基盤で実データ投入
- Phase 3: MiiTel 1クリック架電連携 + 通話録音→AI要約
- Phase 4: Claude API 本番連携（AI営業サマリー / トークスクリプト自動生成）
- Phase 5: FS商談ログ自動引継ぎ本実装（Converted_Account → pipeline__c作成 + Google Calendar招待）
- Phase 6: Slack Webhook URL設定 + Scheduled Flow (日報・週次KPI通知)
- Phase 7: オートダイヤラー / Chrome拡張 / 重複検知Validation Rule
- Phase 8: AI自動推奨アクション（Claude APIでの1on1アドバイス生成）
