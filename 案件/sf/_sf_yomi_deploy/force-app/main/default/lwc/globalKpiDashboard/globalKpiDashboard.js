...
export default class GlobalKpiDashboard extends NavigationMixin(LightningElement) {
    ...
    @track _expandedGroups = new Set();   // P: byname で展開中の学生グループkey（折りたたみ表示）
    // Tab8: 企業別進捗管理
    ...
    // P: 学生グループの展開/折りたたみ（全CA等の大量表示で開いた瞬間のDOMを軽くする）
    handleToggleGroup(e) {
        const key = e.currentTarget.dataset.key;
        if (!key) return;
        const s = new Set(this._expandedGroups);
        if (s.has(key)) s.delete(key); else s.add(key);
        this._expandedGroups = s;
    }
    handleStudentNameSearch(e) {
        // 学生名 部分一致絞り込み（クライアントサイド・スペース全除去で柔軟に）
        // P: 入力のたびに全行再計算・再描画されると重いので300msデバウンス
        const v = (e.target.value || '').replace(/[\s　]+/g, '');
        clearTimeout(this._studentSearchTimer);
        this._studentSearchTimer = setTimeout(() => { this._studentNameQuery = v; }, 300);
    }
    handleCompanySearch(e) {
        const v = (e.target.value || '').replace(/[\s　]+/g, '');
        clearTimeout(this._companySearchTimer);
        this._companySearchTimer = setTimeout(() => { this._companyQuery = v; }, 300);
    }
    ...
    async loadByname() {
        ...
        try {
            ...
            // P: 卒業済みは非表示。選択年度を基準に「その年度以前の卒年」を除外
            //    （年度6月始まり）。例: 2026年度 → 26卒以前を隠し 27卒以降のみ表示。
            //    年度を切り替えれば自動で基準もずれる（各年度共通ロジック）。
            const fyShort = (parseInt(this.selectedYear, 10) || CURRENT_FY) % 100;
            this._bynameRows = recs.filter(r => {
                const g = parseInt(String(r.gradYear), 10);   // "27卒" → 27
                return !isNaN(g) && g > fyShort;
            });
            if (recs.length >= 5000) {
                this.dispatchEvent(new ShowToastEvent({
                    title: '件数上限',
    ...
    get bynameGrouped() {
        ...
        const processedRows = sourceRows.map(r => {
            ...
            return {
                ...
                isSelected,
                // P: ネイティブ<select>はvalueバインドだと先頭固定になる不具合があるため
                //    他タブ同様 selected属性付きオプションで現在値を反映する
                statusOptionsWithSelected: STATUS_OPTIONS.map(o => ({ ...o, selected: o.value === st })),
                accuracyOptionsWithSelected: [{ label: '―', value: '', selected: !r.accuracy }]
                    .concat(ACCURACY_OPTIONS.map(o => ({ ...o, selected: o.value === r.accuracy }))),
                yomiMonthOptionsWithSelected: this._buildYomiMonthOptions(r.yomiMonth ? String(r.yomiMonth).substring(0, 7) : ''),
            };
        });
        ...
        // P: 表示グループが少数（絞り込み済み）なら自動展開、多数なら折りたたみ初期化
        const autoExpand = map.size <= 8;
        const groups = [...map.values()].map(g => {
            ...
            const expanded = autoExpand || this._expandedGroups.has(g.key);
            return {
                ...g,
                totalYomiDisplay: '¥' + Number(Math.round(g.totalYomi)).toLocaleString(),
