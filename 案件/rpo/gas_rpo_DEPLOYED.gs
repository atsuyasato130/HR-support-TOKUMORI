...
function _heroKpiKeys(phases) {
  return ["エントリー", "説明選考会【参加】", "一次面接【合格】", "内定", "内定承諾"].map(lbl => {
    const p = phases.find(x => x.label === lbl);
    return { key: p ? p.key : null, label: lbl };
  });
}

// 全体/当月サマリー用 主要KPI（4指標）
function _summaryKpiKeys(phases) {
  return ["エントリー", "説明選考会【参加】", "一次面接【合格】", "内定承諾"].map(lbl => {
    const p = phases.find(x => x.label === lbl);
    return { key: p ? p.key : null, label: lbl };
  });
}
...
// 実績通過率ベースの着地予想（フェーズ別・現パイプライン・単調非増加）
//   ラダー = main+pass+reserved を phase 定義順（予約も位置決めに含む）。内定承諾は着地対象外（着地は内定まで）。
//   実績通過率 = 各フェーズ累計実績の隣接比
//   着地予想[i] = (i段階以降にいる現アクティブ) + Σ_{手前のアクティブ} × 実績通過率の累積
//     → 現在選考中の学生だけを実績通過率で各段階へ進めた最終到達見込（単調非増加を保証）
//   ※エントリーの着地予想は描画側（212）で「日割りペース」に上書きする
function _landingByPhase(records, phases, targets) {
  const ladder = phases.filter(p =>
    (p.group === "main" || p.group === "pass" || p.group === "reserved") && p.label !== "内定承諾");
  const fStats = aggregateFunnelMonthly(records, phases);
  const cum = ladder.map(p => fStats[p.key]?.TOTAL || 0);

  // 実績通過率 i → i+1（隣接累計比）
  const passR = ladder.map((p, i) =>
    (i >= ladder.length - 1) ? 0 : (cum[i] ? cum[i + 1] / cum[i] : 0));

  // 現在アクティブを各段階へ寄せる（未マップは加算しない＝旧 idx=0 でエントリー過大化していたバグを排除）
  const aStats = aggregateActive(records);
  const active = new Array(ladder.length).fill(0);
  Object.entries(aStats).forEach(([stage, n]) => {
    const s = String(stage).trim();
    let idx = ladder.findIndex(p => p.label === s);
    if (idx < 0 && s.indexOf("一次後面談") >= 0) idx = ladder.findIndex(p => p.label === "一次面接【合格】");
    if (idx < 0) {
      const base = s.replace(/【.+?】/g, "").trim();
      idx = ladder.findIndex(p => p.label.replace(/【.+?】/g, "").trim() === base);
    }
    if (idx >= 0) active[idx] += n;
  });

  // 現パイプライン・カスケード:
  //   landing[i] = (total - CA[i-1]) + P[i]*S[i-1]
  //   P[i]=Π passR(0..i-1), S[i]=Σ_{q<=i} active[q]/P[q], CA[i]=Σ_{q<=i} active[q]
  const total = active.reduce((s, n) => s + n, 0);
  const landing = new Array(ladder.length).fill(0);
  let P = 1, Sprev = 0, CAprev = 0;
  for (let i = 0; i < ladder.length; i++) {
    if (i > 0) P = P * (passR[i - 1] || 0);
    landing[i] = (i === 0) ? total : (total - CAprev) + (P > 0 ? P * Sprev : 0);
    Sprev += (P > 0 ? active[i] / P : 0);
    CAprev += active[i];
  }

  const rows = ladder.map((p, i) => {
    const tgt = Object.values(targets[p.key] || {}).reduce((s, n) => s + n, 0); // 累計目標
    return {
      key: p.key, label: p.label, group: p.group,
      cumActual: cum[i], active: active[i], landing: landing[i],
      target: tgt, ratio: tgt ? landing[i] / tgt : null,
    };
  });
  return { fp: ladder, rows };
}
...
function renderOverviewAll(records, phases, targets, targetsCh) {
  ...
  _sectionHeader(sheet, r++, 6, "■ サマリー（全体ハイライト・累計）");
  sheet.getRange(r, 1, 1, 5).setValues([heroKeys.map(k => k.label)]);
  _tableHeader(sheet, r, 1, 5); r++;
  const heroVals = heroKeys.map(k => k.key ? (fStats[k.key]?.TOTAL || 0) : 0);
  sheet.getRange(r, 1, 1, 5).setValues([heroVals]);
  _tableData(sheet, r, 1, 1, 5);
  sheet.getRange(r, 1, 1, 5).setFontSize(20).setFontWeight("bold").setHorizontalAlignment("center");
  ...
  _sectionHeader(sheet, r++, 2 + FCOLS, "■ 月次ファネル（実績／目標／差分 統合）");
  ...
  activeMonths.forEach(m => {
    ...
    mfRows.push(["", "差分"].concat(act.map((a, i) => _fmtDiff(a - tgt[i]))));
  });
  mfRows.push(["合計", "実績"].concat(totAct));
  mfRows.push(["", "目標"].concat(totTgt.map(t => t || "-")));
  mfRows.push(["", "差分"].concat(totAct.map((a, i) => _fmtDiff(a - totTgt[i]))));
  ...
  for (let i = 2; i < mfRows.length; i += 3) {
    for (let c = 0; c < FCOLS; c++) {
      ...
      const num = (typeof v === "string" && v !== "-") ? parseFloat(v) : NaN;
      let bg = COLOR.bgRow;
      if (!isNaN(num)) bg = num > 0 ? COLOR.ok : num < 0 ? COLOR.alert : COLOR.bgRowAlt;
      sheet.getRange(r + i, 3 + c).setBackground(bg);
    }
  }
  r += mfRows.length + 2;

  // ── ④ 着地予想（フェーズ別・過去実績ベース） ──（承諾の合計着地ブロックは非表示）
  _sectionHeader(sheet, r++, 6, "■ 着地予想（フェーズ別・過去実績ベース）");
...
function renderMonthly(records, phases, targets, targetsCh, targetYM) {
  ...
  r++;
  sheet.getRange(r, 1, 1, WIDE).merge()
    .setValue("※ B2の月を切替えると自動で再描画されます（着地予想は対象期間の最終見込）")
    .setBackground(COLOR.bgRowAlt).setFontColor(COLOR.textDim).setFontSize(10)
    .setFontFamily(FONT).setHorizontalAlignment("left");
  r += 2;
  ...
  // ── ① ハイライトサマリー（選択月） ──
  const heroKeys = _heroKpiKeys(phases);
  _sectionHeader(sheet, r++, 6, `■ サマリー（${target.label}のハイライト）`);
  sheet.getRange(r, 1, 1, 5).setValues([heroKeys.map(k => k.label)]);
  _tableHeader(sheet, r, 1, 5); r++;
  ...
  // （承諾の着地予想ブロックは非表示。着地予想はフェーズ別表のみに集約）
  ...
  const _dayElapsed = _now.getDate();
  const _isCurMonth = (ym === monthKey(_now));
  const landRatios = [];
  const phRows = fp.map(p => {
    const act = fStats[p.key]?.[ym] || 0;
    const tgt = targets[p.key]?.[ym] || 0;            // 当月目標
    const rate = tgt ? act / tgt : null;
    const lp = landMap[p.key];
    let land = lp ? lp.landing : null;
    if (p.group === "reserved" || p.label === "内定承諾") land = null;  // 予約・承諾は着地を出さない
    if (p.label === "エントリー") land = _isCurMonth ? (_dayElapsed > 0 ? act / _dayElapsed * _daysInMonth : act) : act; // 日割りペース
    const landRatio = (land != null && tgt) ? land / tgt : null;        // 着地予想 / 当月目標
    landRatios.push(landRatio);
    return [
  ...
  fp.forEach((p, i) => {
    ...
    if (landRatios[i] != null) sheet.getRange(r + i, 7).setBackground(rateToColor(landRatios[i]));
  });
  r += phRows.length + 2;
