...
const COLOR = {
  ...
  bgRowAlt:  "#F5F5F5",
  totalBg:   "#ECEFF1",
  sumRow:    "#CFD8DC",  // 合計行専用（見出しtotalBgより一段濃いスレート。データ行と明確に区別）
  bgInsight: "#FCF5D9",
  fgWhite:   "#FFFFFF",
  textMain:  "#212121",
  textDim:   "#757575",
...
function loadRawRecords() {
  ...
  for (let r = 2; r < data.length; r++) {
    ...
    const rec = {
      ...
      channel:      (() => {
        const _raw = String(row[colIdx["エントリー経路"]] || "").trim();
        const _norm = _normalizeChannel(_raw);
        if (_norm === "紹介会社") {
          const _agent = String(row[colIdx["紹介会社"]] || "").trim();  // 新列: 紹介会社エージェント名
          if (_agent) return _agent;  // どこのエージェントか判別できるよう社名で識別
        }
        return _norm;
      })(),
...
// 月次ファネル（自列＝予約日/合格日/参加日 基準）: 各フェーズ「自体の列」の日付の月で計上。
//   ※ ・到達日(=状態到達の記録日)ではなく、予約/参加/合格などフェーズ列の実日付(rec.scheduled)を使う。211/212用。
function aggregateFunnelMonthlyOwn(records, phases) {
  const result = {};
  phases.forEach(p => { result[p.key] = { TOTAL: 0 }; });
  records.forEach(rec => {
    phases.forEach(p => {
      const dt = rec.scheduled[p.key];
      if (!dt) return;
      const mk = monthKey(dt);
      result[p.key][mk] = (result[p.key][mk] || 0) + 1;
      result[p.key].TOTAL += 1;
...
// 月次版 評価ランク分布: 当該フェーズの実施日(自列 scheduled[p.key]) が ym のレコードだけを集計
function aggregateRankingMonth(records, phases, ym) {
  const targetPhases = phases.filter(p => p.group === "main");
  const result = {};
  targetPhases.forEach(p => {
    result[p.key] = { S: 0, A: 0, B: 0, C: 0, D: 0, TOTAL: 0, label: p.label };
  ...
  records.forEach(rec => {
    targetPhases.forEach(p => {
      const rank = rec.evals[p.key];
      if (!RANKS.includes(rank)) return;
      const dt = rec.scheduled[p.key];           // フェーズ自体の列の日付（参加日等）
      if (!dt || monthKey(dt) !== ym) return;
      result[p.key][rank] = (result[p.key][rank] || 0) + 1;
      result[p.key].TOTAL += 1;
    });
  });
  return result;
...
// ファネル通過率＆ボトルネック分析（分析の鋭さ）: 主要進行フェーズの隣接通過率(実績・自列基準)を
//   120目標の通過率と突合し、目標を下回る乖離が最大の遷移＝ボトルネックを返す。ym省略=累計/指定=当月。
function _funnelConversion(records, phases, targets, ym) {
  const SEQ = ["エントリー", "説明選考会【参加】", "説明選考会【合格】",
               "一次面接【参加】", "一次面接【合格】",
               "二次面接【参加】", "二次面接【合格】",
               "最終選考【参加】", "内定", "内定承諾"];
  const fStats = aggregateFunnelMonthlyOwn(records, phases);
  const cntOf = (label) => {
    const p = phases.find(x => x.label === label);
    if (!p) return 0;
    return ym ? (fStats[p.key]?.[ym] || 0) : (fStats[p.key]?.TOTAL || 0);
  };
  const tgtOf = (label) => {
    const p = phases.find(x => x.label === label);
    if (!p) return 0;
    return ym ? (targets[p.key]?.[ym] || 0)
              : Object.values(targets[p.key] || {}).reduce((s, n) => s + n, 0);
  };
  const seq = SEQ.filter(l => phases.some(p => p.label === l));
  const steps = [];
  for (let i = 0; i < seq.length - 1; i++) {
    const aFrom = cntOf(seq[i]), aTo = cntOf(seq[i + 1]);
    const tFrom = tgtOf(seq[i]), tTo = tgtOf(seq[i + 1]);
    const actRate = aFrom > 0 ? aTo / aFrom : null;
    const tgtRate = tFrom > 0 ? tTo / tFrom : null;
    const gap = (actRate !== null && tgtRate !== null) ? actRate - tgtRate : null;
    steps.push({ from: seq[i], to: seq[i + 1], aFrom, aTo, actRate, tgtRate, gap });
  }
  let bottleneck = null;
  steps.forEach(s => {
    if (s.gap !== null && s.gap < 0 && s.aFrom >= 3) {
      if (!bottleneck || s.gap < bottleneck.gap) bottleneck = s;
    }
  });
  return { steps, bottleneck };
}

// チャネル/エージェント別の"質"（量でなく転換率）: エントリー→一次合格／→内定 の効率%でランキング
function _channelQuality(records, phases) {
  const ch = aggregateByChannel(records, phases);
  const passK = phases.find(p => p.label === "一次面接【合格】")?.key;
  const offerK = phases.find(p => p.label === "内定")?.key;
  const acceptK = phases.find(p => p.label === "内定承諾")?.key;
  return Object.entries(ch)
    .filter(([, s]) => s.entry > 0)
    .map(([name, s]) => {
      const pass1 = passK ? (s.byPhase[passK] || 0) : 0;
      const offer = offerK ? (s.byPhase[offerK] || 0) : 0;
      const accept = acceptK ? (s.byPhase[acceptK] || 0) : 0;
      return {
        name, entry: s.entry, pass1, offer, accept,
        byPhase: s.byPhase,
        pass1Rate: s.entry ? pass1 / s.entry : 0,
        offerRate: s.entry ? offer / s.entry : 0,
      };
    })
    .sort((a, b) => b.offerRate - a.offerRate || b.pass1Rate - a.pass1Rate || b.entry - a.entry);
}
...
function renderOverview(records, phases, targets, targetsCh) {
  ...
  if (activeRows.length) {
    ...
    sheet.getRange(r + activeRows.length - 1, 10, 1, 2)
      .setBackground(COLOR.sumRow).setFontWeight("bold");
  }
...
function renderBySchool(records, phases) {
  ...
  // ── 大学トップ5（エントリー数）── 上部に自動更新で表示（8行目までに収める）
  let r = 2;
  const _t5pGD  = phases.find(p => p.label === "説明選考会【合格】")?.key;
  const _t5p1   = phases.find(p => p.label === "一次面接【合格】")?.key;
  const _t5acc  = phases.find(p => p.label === "内定承諾")?.key;
  _sectionHeader(sheet, r++, 6, "■ 大学トップ5（エントリー数）");
  sheet.getRange(r, 1, 1, 6).setValues([["順位", "大学名", "エントリー", "説明選考会【合格】", "一次面接【合格】", "内定承諾"]]);
  _tableHeader(sheet, r, 1, 6); r++;
  const _top5 = Object.entries(schoolStats)
    .filter(([, s]) => s.entry > 0)
    .sort((a, b) => b[1].entry - a[1].entry || (a[0] < b[0] ? -1 : 1))
    .slice(0, 5);
  if (_top5.length) {
    const _t5rows = _top5.map(([sch, s], i) => [
      i + 1, sch, s.entry,
      _t5pGD ? (s.byPhase[_t5pGD] || 0) : 0,
      _t5p1 ? (s.byPhase[_t5p1] || 0) : 0,
      _t5acc ? (s.byPhase[_t5acc] || 0) : 0,
    ]);
    sheet.getRange(r, 1, _t5rows.length, 6).setValues(_t5rows);
    _tableData(sheet, r, 1, _t5rows.length, 6);
    r += _t5rows.length;
  }
  r += 1;
...
function renderEntryChannel(records, phases, targetsCh) {
  ...
  if (rows.length) {
    ...
    rows.push(totalRow);
    colors.push(Array(totalCols).fill(COLOR.sumRow));
    ...
    sheet.getRange(r + rows.length - 1, 1, 1, totalCols)
      .setFontWeight("bold").setBackground(COLOR.sumRow);
  }
  r += rows.length + 2;
...
function generateOverviewInsights(records, phases, targets, targetsCh) {
  ...
  const fStats = aggregateFunnelMonthlyOwn(records, phases);  // 自列(予約日/合格日)基準＝ダッシュボードと統一
  const cStats = aggregateChannelMonthly(records);
  const atr = aggregateAttrition(records, phases);
  const schoolStats = aggregateBySchool(records, phases);
  const landing = aggregateLanding(records, phases, targets);
...
function onOpen() {
  SpreadsheetApp.getUi().createMenu("RPO")
    ...
    .addSeparator()
    .addItem("週次レポート プレビュー", "previewWeeklyReportV2")
    .addItem("週次レポート 送信", "sendWeeklyReportV2")
    .addItem("週次トリガー設定（水9時）", "setWeeklyTriggerV2")
    .addItem("日次レポート プレビュー", "previewDailyReport")
    .addItem("日次レポート 送信", "sendDailyReport")
    .addSeparator()
    .addItem("月次レポートDoc 前月分生成", "previewMonthlyDocV2")
    .addItem("月次レポートDoc 過去月生成", "generatePastMonthlyDocV2")
    .addItem("月次トリガー設定（毎月1日）", "setMonthlyTriggerV2")
    .addSeparator()
    .addItem("レポート出力タブ 再生成（前月）", "generateReportTabPrevMonth")
    .addItem("レポート出力タブ（過去月を指定）", "generateReportTabForMonth")
    .addItem("レポートタブ 月次トリガー", "setReportTabMonthlyTrigger")
    .addItem("レポートタブ 週次トリガー", "setReportTabWeeklyTrigger")
    .addSeparator()
    .addItem("毎朝9時 再集計トリガー設定", "setDailyRebuildTrigger")
    .addToUi();
}
...
// 今後7日に予約されているイベント数
// 直近7日(過去1週間)の実績: 各フェーズの「自列=実施日」が過去7日に入る件数。週次を未来予定→過去実績に。
function aggregatePast7DaysActuals(records, phases, today) {
  const endMs = today.getTime();
  const startMs = new Date(today.getFullYear(), today.getMonth(), today.getDate()).getTime() - 7 * 86400000;
  const inWk = (d) => d instanceof Date && d.getTime() >= startMs && d.getTime() <= endMs;
  const countOwn = (label) => {
    const p = phases.find(x => x.label === label);
    if (!p) return 0;
    let c = 0;
    records.forEach(rec => { if (inWk(rec.scheduled[p.key])) c++; });
    return c;
  };
  return {
    newEntry:   countOwn("エントリー"),
    briefing:   countOwn("説明選考会【参加】"),
    interview1: countOwn("一次面接【参加】"),
    interview2: countOwn("二次面接【参加】"),
    interviewF: countOwn("最終選考【参加】"),
    offer:      countOwn("内定"),
  };
}
...
function composeWeeklyBlocksV2() {
  ...
  const sched = aggregatePast7DaysActuals(records, phases, today);
  const actions = extractCriticalActions(records, phases, today, 10);
  const weekStart = new Date(today.getTime() - 6 * 86400000);
  const periodLabel = `${formatDate(weekStart)} 〜 ${formatDate(today)}`;

  const blocks = [];
  blocks.push({ type: "header", text: { type: "plain_text", text: `${company} 採用週次レポート (${periodLabel})` } });

  // 直近1週間の実績（過去7日・自列=実施日ベース）
  const schedLines = [
    `🆕 *新規エントリー*: ${sched.newEntry}名`,
    `🎤 *説明会参加*: ${sched.briefing}名`,
    `📋 *1次面接参加*: ${sched.interview1}名`,
    `📋 *2次面接参加*: ${sched.interview2}名`,
    `🎯 *最終面接参加*: ${sched.interviewF}名`,
    `🏆 *内定*: ${sched.offer}名`,
  ];
  blocks.push({ type: "section", text: { type: "mrkdwn", text: `*■ 直近1週間の実績（${periodLabel}）*\n${schedLines.join("\n")}` } });
  ...
  // ── 分析: ファネル通過率＆ボトルネック / 着地ペース（自列基準）──
  let bottleneckMsg = "", paceMsg = "";
  try {
    const tF = loadTargetsFunnel(phases);
    const ym = monthKey(today);
    const fmtR = (x) => (x === null || x === undefined) ? "-" : Math.round(x * 100) + "%";
    const conv = _funnelConversion(records, phases, tF, ym);
    const convLines = conv.steps.filter(s => s.actRate !== null).map(s => {
      const tgt = s.tgtRate !== null ? `（目標 ${fmtR(s.tgtRate)}）` : "";
      const mark = (s.gap !== null && s.gap < -0.05) ? " ⚠️" : "";
      return `• ${s.from}→${s.to}: *${fmtR(s.actRate)}*${tgt}${mark}`;
    });
    if (convLines.length) {
      let t = `*■ ファネル通過率（当月・対目標）*\n${convLines.join("\n")}`;
      if (conv.bottleneck) {
        const b = conv.bottleneck;
        bottleneckMsg = `${b.from}→${b.to}(実績${fmtR(b.actRate)}/目標${fmtR(b.tgtRate)})`;
        t += `\n🔴 *ボトルネック*: ${b.from}→${b.to}（実績 ${fmtR(b.actRate)} ＜ 目標 ${fmtR(b.tgtRate)}）`;
      }
      blocks.push({ type: "section", text: { type: "mrkdwn", text: t } });
    }
    const landP = _landingByPhase(records, phases, tF);
    const offerRow = landP.rows.find(x => x.label === "内定");
    if (offerRow && offerRow.target > 0) {
      const ratio = offerRow.ratio || 0;
      const judge = ratio >= 1 ? "✅ 達成ペース" : ratio >= 0.8 ? "🟡 やや未達" : "🔴 未達ペース";
      paceMsg = `内定着地 ${offerRow.landing.toFixed(1)}/${offerRow.target}（${Math.round(ratio * 100)}%）`;
      blocks.push({ type: "section", text: { type: "mrkdwn", text: `*■ 着地ペース（内定・予測 vs 期間目標）*\n• 着地予測 *${offerRow.landing.toFixed(1)}名* ／ 期間目標 ${offerRow.target}名 ＝ ${Math.round(ratio * 100)}%　${judge}\n_※現在選考中の学生を実績通過率で進めた予測_` } });
    }
  } catch (e) { Logger.log("weekly analysis skip: " + e.message); }
  ...
  try {
    ...
    const offerKey = phases.find(p => p.label === "内定")?.key;
    if (entryP) {
      const wkStart = new Date(today.getFullYear(), today.getMonth(), today.getDate()).getTime() - 7 * 86400000;
      const sAgg = aggregateBySchool(records, phases);
      const wkBySchool = {};
      records.forEach(rec => {
        const d = rec.arrivals[entryP.key];
        if (d instanceof Date && d.getTime() >= wkStart) { const s = rec.school || "（未記入）"; wkBySchool[s] = (wkBySchool[s] || 0) + 1; }
      });
      const top = Object.entries(sAgg).filter(([s]) => s && s !== "（未記入）")
        .sort((a, b) => b[1].entry - a[1].entry).slice(0, 6);
      if (top.length) {
        const lines = top.map(([s, st]) => `• *${s}*　今週 ${wkBySchool[s] || 0}名 ／ 累計 ${st.entry}名${offerKey && st.byPhase[offerKey] ? `（内定${st.byPhase[offerKey]}）` : ""}`);
        blocks.push({ type: "section", text: { type: "mrkdwn", text: `*■ 大学別 主要校（今週新規／累計）*\n${lines.join("\n")}` } });
      }
    }
  } catch (e) { Logger.log("weekly school skip: " + e.message); }

  // ── 経路別 質（効率・投資配分の示唆）★深掘り ──
  try {
    const cq = _channelQuality(records, phases).filter(c => c.entry >= 3);
    if (cq.length) {
      const best = cq.slice(0, 3).map(c => `• *${c.name}*　一次合格率 ${Math.round(c.pass1Rate * 100)}% ／ 内定率 ${Math.round(c.offerRate * 100)}%（エントリー${c.entry}）`);
      let t = `*■ 経路別 質（効率上位・投資配分の判断材料）*\n${best.join("\n")}`;
      if (cq.length > 3) {
        const worst = cq[cq.length - 1];
        t += `\n🔻 効率最下位: *${worst.name}*（一次合格率 ${Math.round(worst.pass1Rate * 100)}% ／ エントリー${worst.entry}）→ 配分見直し検討`;
      }
      blocks.push({ type: "section", text: { type: "mrkdwn", text: t } });
    }
  } catch (e) { Logger.log("weekly channel-quality skip: " + e.message); }

  // 一言
  const summarySrc = [
    `直近1週間実績: 新規エントリー${sched.newEntry} / 説明会参加${sched.briefing} 1次参加${sched.interview1} 2次参加${sched.interview2} 最終参加${sched.interviewF} 内定${sched.offer}`,
    `現在ステータス: ` + statusOrder.slice(0, 8).map(p => `${p.label}=${cumActive[p.key]||0}`).join(", "),
    `要対応: ${actions.length}名`,
    bottleneckMsg ? `ファネルのボトルネック: ${bottleneckMsg}` : "",
    paceMsg ? `着地ペース: ${paceMsg}` : "",
  ].filter(Boolean).join("\n");
...
function _monthlyKpiSnapshot(records, phases, year, month) {
  ...
  const offerP = phases.find(p => p.label === "内定");
  const acceptP = phases.find(p => p.label === "内定承諾");
  // 合格（pass）フェーズを正規表現で頑健に取得（KPIサマリーを合格ベースにするため）
  const passCnt = (re) => { const p = phases.find(x => x.group === "pass" && re.test(x.label)); return p ? (fStats[p.key]?.[ym] || 0) : 0; };
  return {
    ym,
    ...
    passBrief:  passCnt(/説明選考会.*合格|説明会.*合格/),
    passI1:     passCnt(/一次.*合格|1次.*合格/),
    passIntern: passCnt(/インターン.*合格/),
    passI2:     passCnt(/二次.*合格|2次.*合格/),
  };
}
...
function _monthlyTrendData(records, phases, year, month, monthsBack) {
  ...
  const entryP = phases.find(p => p.label === "エントリー");
  const offerP = phases.find(p => p.label === "内定");
  // 全フェーズ推移（合格ベースのKPI段）。series は全フェーズグラフ用、entries/offers は後方互換
  const seriesDefs = [
    { label: "エントリー",     src: "エントリー" },
    { label: "説明会合格",     src: "説明選考会【合格】" },
    { label: "一次合格",       src: "一次面接【合格】" },
    { label: "インターン合格", src: "1dayインターン【合格】" },
    { label: "二次合格",       src: "二次面接【合格】" },
    { label: "内定",           src: "内定" },
  ].map(d => ({ label: d.label, key: phases.find(p => p.label === d.src) ? phases.find(p => p.label === d.src).key : null, data: [] }));
  for (let i = monthsBack - 1; i >= 0; i--) {
    ...
    seriesDefs.forEach(s => s.data.push(s.key ? (fStats[s.key]?.[ym] || 0) : 0));
  }
  return { labels, entries, offers, series: seriesDefs.map(s => ({ label: s.label, data: s.data })) };
}
...
function _appendDocTable(body, headers, rows) {
  ...
  for (let c = 0; c < headers.length; c++) {
    try {
      const cell = table.getRow(0).getCell(c);
      cell.setBackgroundColor(DOC_INK);
...
function generateMonthlyDocV2(targetMonth) {
  ...
  try {
    // タイトル（黒帯）
    _docTitleBand_(body, docName, `${company} 新卒採用　／　${ymLabel}　／　生成日: ${formatDate(now)}`);
    body.appendParagraph("");

    // KPIサマリー（合格ベース・前月比付き）= 最初のサマリー
    //   エントリー → 説明会合格 → 一次合格 → インターン合格 → 二次合格 → 内定
    _docAppendKpiBlock(body, [
      { label: "エントリー",     value: currKpi.entry,      diff: _diffLabel(currKpi.entry, prevKpi.entry) },
      { label: "説明会合格",     value: currKpi.passBrief,  diff: _diffLabel(currKpi.passBrief, prevKpi.passBrief) },
      { label: "一次合格",       value: currKpi.passI1,     diff: _diffLabel(currKpi.passI1, prevKpi.passI1) },
      { label: "インターン合格", value: currKpi.passIntern, diff: _diffLabel(currKpi.passIntern, prevKpi.passIntern) },
      { label: "二次合格",       value: currKpi.passI2,     diff: _diffLabel(currKpi.passI2, prevKpi.passI2) },
      { label: "内定",           value: currKpi.offer,      diff: _diffLabel(currKpi.offer, prevKpi.offer) },
    ]);
    body.appendParagraph(`※ 各指標の上段＝${ymLabel}の実績（合格ベース）、下段＝前月比（増減・増減率）`).editAsText().setForegroundColor(DOC_MUTED).setItalic(true).setFontSize(9);
    body.appendParagraph("");

    // 担当者コメント（編集してください）— ルールベース下書き、担当者が自由に編集
    ...
    const summaryPts = [];
    try {
      const conv = _funnelConversion(records, phases, targets, currKpi.ym);
      if (conv.bottleneck) {
        const b = conv.bottleneck;
        summaryPts.push(`ボトルネックは「${b.from}→${b.to}」（実績 ${_fmtPct(b.actRate)} ＜ 目標 ${_fmtPct(b.tgtRate)}）。ここの改善が最優先。`);
      }
      const landP = _landingByPhase(records, phases, targets);
      const offerRow = landP.rows.find(x => x.label === "内定");
      if (offerRow && offerRow.target > 0) {
        const ratio = offerRow.ratio || 0;
        const j = ratio >= 1 ? "達成ペース" : ratio >= 0.8 ? "やや未達ペース" : "未達ペース";
        summaryPts.push(`内定の着地予測 ${offerRow.landing.toFixed(1)}名／期間目標 ${offerRow.target}名（${Math.round(ratio * 100)}%）＝${j}。`);
      }
      const cq = _channelQuality(records, phases);
      if (cq.length) {
        const top = cq[0];
        summaryPts.push(`チャネル質トップは ${top.name}（エントリー→一次合格 ${_fmtPct(top.pass1Rate)}）。量と質の両面で配分を検討。`);
      }
      const rkE = Object.values(aggregateRanking(records, phases)).filter(s => s.TOTAL > 0)
        .map(s => ({ label: s.label, sa: s.TOTAL ? (s.S + s.A) / s.TOTAL : 0, n: s.TOTAL }));
      if (rkE.length) summaryPts.push(`評価の質：${rkE[0].label}のS+A率 ${_fmtPct(rkE[0].sa)}（n=${rkE[0].n}）。`);
    } catch (e) { Logger.log("exec summary skip: " + e.message); }
    if (!summaryPts.length) summaryPts.push("当月の主要指標に特記事項なし。");
    summaryPts.forEach(p => body.appendParagraph("・" + p));
    ...
    try {
      ...
      const blob = _qcLineSmart_(trend.labels, trend.series, "全フェーズの月次推移");
      if (blob) body.appendImage(blob).setWidth(500).setHeight(230);
    } catch (e) { Logger.log("trend chart skip: " + e.message); }

    // 追加グラフ: ファネル / 媒体・経路別 / 大学別（★見やすさ強化）
    try { _rpoAppendDocCharts_(body, records, phases, currKpi.ym); } catch (e) { Logger.log("doc charts skip: " + e.message); }
    ...
    // ─── 単月詳細 ───（強制改ページは空白ページの原因になるため使わず自然送り）
    body.appendParagraph("");
    _docH_(body, `${ymLabel} 単月詳細`);
    ...
    const phaseRows = orderedPhases.map(p => {
      ...
      return [p.label, tgt ? String(tgt) : "-", String(c), _targetDiffLabel(c, tgt), String(pv), _diffLabel(c, pv)];
    });
    _docH_(body, "全選考フェーズ実績 (当月)");
    _appendDocTable(body, ["フェーズ", "目標", `${month}月(実績)`, "差分(達成率)", `${prevDate.getMonth()+1}月`, "前月比"], phaseRows);

    // ① 媒体・エージェント別 応募元内訳 (当月 / 累計 / 構成比) ＋ 当月目標差分
    //    媒体系セクションはここに一本化（上部の「媒体・経路別（当月構成）」チャートは廃止＝重複解消）。
    //    エージェント(当月0でも累計あり)も見えるよう累計>0で抽出し、構成グラフも累計ベース。
    ...
    const sourceRows = Object.entries(srcStats)
      .filter(([, v]) => v.total > 0)
      .sort((a, b) => b[1].total - a[1].total || b[1].curr - a[1].curr)
      .map(([name, v]) => {
    ...
    if (sourceRows.length) {
      ...
      body.appendParagraph(" ");
      _docH_(body, "媒体・エージェント別（応募元内訳）");
      // 構成グラフ（累計ベース＝エージェントも表示・上位10）
      try {
        const ck = Object.entries(srcStats).filter(([, v]) => v.total > 0)
          .sort((a, b) => b[1].total - a[1].total).slice(0, 10);
        const cb = _qcBarH_(ck.map(x => x[0]), ck.map(x => x[1].total), "媒体・エージェント別 応募数（累計）");
        if (cb) body.appendImage(cb).setWidth(520).setHeight(64 + ck.length * 26);
      } catch (e) { Logger.log("media chart skip: " + e.message); }
    ...
    // 大学別Top10（フェーズ進捗付き・★深掘り強化）
    const schoolStats = aggregateBySchool(records, phases);
    const _spk = (label) => phases.find(p => p.label === label)?.key;
    const _sBrief = _spk("説明選考会【参加】"), _sI1 = _spk("一次面接【参加】"), _sFin = _spk("最終選考【参加】"), _sOffer = _spk("内定");
    const _sCell = (s, k) => String(k ? (s.byPhase[k] || 0) : 0);
    const _sRate = (s, k) => s.entry ? Math.round((k ? (s.byPhase[k] || 0) : 0) / s.entry * 100) + "%" : "-";
    const schoolRows = Object.entries(schoolStats)
      .filter(([, s]) => s.entry > 0)
      .map(([sch, s]) => [sch, String(s.entry), _sCell(s, _sBrief), _sCell(s, _sI1), _sCell(s, _sFin), _sCell(s, _sOffer), _sRate(s, _sOffer)])
      .sort((a, b) => parseInt(b[1]) - parseInt(a[1]))
      .slice(0, 10);
    if (schoolRows.length) {
      body.appendParagraph(" ");
      _docH_(body, "大学別エントリーTop10 (累計・フェーズ進捗)");
      _appendDocTable(body, ["大学", "エントリー", "説明会参加", "一次参加", "最終参加", "内定", "内定率"], schoolRows);
    }

    // ◆ ファネル通過率（当月・対目標）
    try {
      const conv = _funnelConversion(records, phases, targets, currKpi.ym);
      const convRows = conv.steps.filter(s => s.actRate !== null).map(s => {
        const gap = (s.gap !== null) ? ((s.gap >= 0 ? "+" : "") + Math.round(s.gap * 100) + "pt") : "-";
        const judge = (s.gap === null) ? "-" : (s.gap >= 0 ? "○ 目標以上" : s.gap >= -0.05 ? "△ ほぼ目標" : "× 要改善");
        return [`${s.from}→${s.to}`, _fmtPct(s.actRate), s.tgtRate !== null ? _fmtPct(s.tgtRate) : "-", gap, judge];
      });
      if (convRows.length) {
        body.appendParagraph(" ");
        _docH_(body, "ファネル通過率 (当月・対目標)");
        _appendDocTable(body, ["遷移", "実績通過率", "目標通過率", "差", "判定"], convRows);
      }
    } catch (e) { Logger.log("conv section skip: " + e.message); }

    // ◆ チャネル・エージェント別 質（量でなく転換率）
    try {
      const cq = _channelQuality(records, phases);
      // 全選考フェーズの到達数（エントリー比%）を表示
      const cqPhaseDefs = [
        { h: "説明会合格", label: "説明選考会【合格】" },
        { h: "一次合格",   label: "一次面接【合格】" },
        { h: "二次合格",   label: "二次面接【合格】" },
        { h: "最終参加",   label: "最終選考【参加】" },
        { h: "内定",       label: "内定" },
      ].map(d => ({ h: d.h, key: phases.find(p => p.label === d.label) ? phases.find(p => p.label === d.label).key : null }));
      const cqHeader = ["経路", "エントリー"].concat(cqPhaseDefs.map(d => d.h + "(率)"));
      const cqRows = cq.map(c => [c.name, String(c.entry)].concat(
        cqPhaseDefs.map(d => { const n = d.key ? (c.byPhase[d.key] || 0) : 0; return `${n} (${c.entry ? Math.round(n / c.entry * 100) : 0}%)`; })
      ));
      if (cqRows.length) {
        body.appendParagraph(" ");
        _docH_(body, "チャネル・エージェント別 質 (累計・全フェーズ転換率)");
        _appendDocTable(body, cqHeader, cqRows);
      }
    } catch (e) { Logger.log("channel quality section skip: " + e.message); }

    // ◆ 総合評価分布（S/A/B/C/D・累計）
    try {
      // 全選考フェーズ（各【参加】段）を常時表示。エントリーは評価対象外なので除外。0件でも行を出す。
      const rkRows = Object.values(aggregateRanking(records, phases)).filter(s => s.label !== "エントリー").map(s => {
        const sa = s.TOTAL ? (s.S + s.A) / s.TOTAL : 0;
        return [s.label, String(s.S), String(s.A), String(s.B), String(s.C), String(s.D), Math.round(sa * 100) + "%"];
      });
      if (rkRows.length) {
        body.appendParagraph(" ");
        _docH_(body, "総合評価分布 (S/A/B/C/D・累計)");
        _appendDocTable(body, ["フェーズ", "S", "A", "B", "C", "D", "S+A率"], rkRows);
      }
    } catch (e) { Logger.log("rank section skip: " + e.message); }
    ...
    const actItems = [];
    try {
      const convA = _funnelConversion(records, phases, targets, currKpi.ym);
      if (convA.bottleneck) {
        const b = convA.bottleneck;
        const gapPt = b.gap !== null ? Math.round(Math.abs(b.gap) * 100) : null;
        actItems.push(`最優先ボトルネック「${b.from}→${b.to}」：実績通過率 ${_fmtPct(b.actRate)}（目標 ${_fmtPct(b.tgtRate)}${gapPt !== null ? `／${gapPt}pt不足` : ""}）。選考設計・面接官アサイン・日程調整リードタイムを見直し、目標通過率への回復を最優先で図る。`);
      }
      const landA = _landingByPhase(records, phases, targets);
      const offerA = landA.rows.find(x => x.label === "内定");
      if (offerA && offerA.target > 0) {
        const short = Math.max(0, offerA.target - offerA.landing);
        actItems.push(`内定の着地予測 ${offerA.landing.toFixed(1)}名 vs 期間目標 ${offerA.target}名（不足 ${short.toFixed(1)}名）。不足分から逆算し、必要エントリー数とチャネル配分を再設定する。`);
      }
      const cqA = _channelQuality(records, phases);
      if (cqA.length) {
        const best = cqA[0];
        actItems.push(`質トップのチャネルは ${best.name}（エントリー→内定 ${_fmtPct(best.offerRate)}）。予算・工数をここへ重点配分し、低転換チャネルは絞る。`);
      }
    } catch (e) { Logger.log("action build skip: " + e.message); }
    // 補足: 自動インサイト（警戒/リスク）
    insights.slice(0, 3).forEach(i => actItems.push(`${i.title}: ${i.text}`));
    if (!actItems.length) body.appendParagraph("（特記なし）");
    else actItems.forEach(l => body.appendListItem(l));
...
// サマリー/ハイライト 共通指標セット（各フェーズ「合格」中心の7指標）
// ※ 2daysインターンは廃止（かんりくんから消滅）のため除外
const SUMMARY_LABELS = [
  "エントリー", "説明選考会【合格】", "一次面接【合格】",
  "1dayインターン【合格】", "二次面接【合格】",
  "内定", "内定承諾",
];
...
function _landingByPhase(records, phases, targets) {
  ...
  const fStats = aggregateFunnelMonthlyOwn(records, phases);  // 自列(予約日/合格日)基準
  const cum = {};
...
function _byKeyPhaseForMonth(records, phases, ym, keyFn) {
  ...
  records.forEach(rec => {
    ...
    phases.forEach(p => {
      const dt = rec.scheduled[p.key];  // 自列(予約日/合格日/参加日)基準
...
function renderOverviewAll(records, phases, targets, targetsCh) {
  ...
  const fStats   = aggregateFunnelMonthlyOwn(records, phases);  // 自列(予約日/合格日/参加日)基準
  const aStats   = aggregateActive(records);
  ...
  // ── ③ 月次ファネル（目標／実績／達成率 統合・実績ベース＝各フェーズ到達日の月で計上） ──
  //   ※ 旧コホート集計（エントリー月一括計上）を実績ベース(fStats)へ変更。合計行は■全体サマリー累計と一致する。
  const months = buildMonthList();
  const activeMonths = months.filter(m =>
    fp.some(p => (fStats[p.key]?.[m.ym] || 0) > 0 || (targets[p.key]?.[m.ym] || 0) > 0));
  if (activeMonths.length === 0) activeMonths.push(months[0]);

  _sectionHeader(sheet, r++, 2 + FCOLS, "■ 月次ファネル（目標／実績／達成率 統合）");
  sheet.getRange(r, 1, 1, 2 + FCOLS).setValues([["月", "区分"].concat(fp.map(p => p.label))]);
  _tableHeader(sheet, r, 1, 2 + FCOLS); r++;

  const mfRows = [];
  const totAct = fp.map(p => fStats[p.key]?.TOTAL || 0);
  const totTgt = fp.map(p => Object.values(targets[p.key] || {}).reduce((s, n) => s + n, 0));
  activeMonths.forEach(m => {
    const act = fp.map(p => fStats[p.key]?.[m.ym] || 0);
    const tgt = fp.map(p => targets[p.key]?.[m.ym] || 0);
    mfRows.push([m.label, "目標"].concat(tgt.map(t => t || "-")));
    mfRows.push(["", "実績"].concat(act));
    mfRows.push(["", "達成率"].concat(act.map((a, i) => tgt[i] ? _fmtPct(a / tgt[i]) : "-")));
  });
  mfRows.push(["合計", "目標"].concat(totTgt.map(t => t || "-")));
  mfRows.push(["", "実績"].concat(totAct));
  ...
  // 合計3行（目標/実績/達成率）を合計専用色で強調
  sheet.getRange(r + mfRows.length - 3, 1, 3, 2 + FCOLS).setBackground(COLOR.sumRow).setFontWeight("bold");
  // 達成率行に色付け（3行周期の3行目）：達成率に応じて緑/黄/赤（合計の達成率セルもrate色で上書き）
  ...
  sheet.getRange(r + aRows.length - 1, 1, 1, 2).setBackground(COLOR.sumRow).setFontWeight("bold");
  r += aRows.length + 2;
  ...
  // ── ⑧ フェーズ別 総合評価分布（S/A/B/C/D・累計） ──
  _sectionHeader(sheet, r++, 7, "■ フェーズ別 総合評価分布（S/A/B/C/D・累計）");
  sheet.getRange(r, 1, 1, 7).setValues([["フェーズ", "S", "A", "B", "C", "D", "S+A率"]]);
  _tableHeader(sheet, r, 1, 7); r++;
  const _rk = aggregateRanking(records, phases);
  const _rkE = Object.entries(_rk).filter(([, s]) => s.TOTAL > 0);
  if (_rkE.length) {
    const _rkRows = _rkE.map(([, s]) => [s.label, s.S, s.A, s.B, s.C, s.D, _fmtPct(s.TOTAL ? (s.S + s.A) / s.TOTAL : null)]);
    sheet.getRange(r, 1, _rkRows.length, 7).setValues(_rkRows);
    _tableData(sheet, r, 1, _rkRows.length, 7);
    _rkE.forEach(([, s], i) => sheet.getRange(r + i, 7).setBackground(rateToColor(s.TOTAL ? (s.S + s.A) / s.TOTAL : null)));
    r += _rkRows.length;
  } else {
    sheet.getRange(r, 1, 1, 7).setValues([["（評価データなし）", "", "", "", "", "", ""]]);
    _tableData(sheet, r, 1, 1, 7); r++;
  }
  r += 2;
...
function renderMonthly(records, phases, targets, targetsCh, targetYM) {
  ...
  // ── ⑦ フェーズ別 総合評価分布（S/A/B/C/D・当月） ──
  _sectionHeader(sheet, r++, 7, `■ フェーズ別 総合評価分布（S/A/B/C/D・${target.label}）`);
  sheet.getRange(r, 1, 1, 7).setValues([["フェーズ", "S", "A", "B", "C", "D", "S+A率"]]);
  _tableHeader(sheet, r, 1, 7); r++;
  const _rkM = aggregateRankingMonth(records, phases, ym);
  const _rkME = Object.entries(_rkM).filter(([, s]) => s.TOTAL > 0);
  if (_rkME.length) {
    const _rkMRows = _rkME.map(([, s]) => [s.label, s.S, s.A, s.B, s.C, s.D, _fmtPct(s.TOTAL ? (s.S + s.A) / s.TOTAL : null)]);
    sheet.getRange(r, 1, _rkMRows.length, 7).setValues(_rkMRows);
    _tableData(sheet, r, 1, _rkMRows.length, 7);
    _rkME.forEach(([, s], i) => sheet.getRange(r + i, 7).setBackground(rateToColor(s.TOTAL ? (s.S + s.A) / s.TOTAL : null)));
    r += _rkMRows.length;
  } else {
    sheet.getRange(r, 1, 1, 7).setValues([["（該当月の評価データなし）", "", "", "", "", "", ""]]);
    _tableData(sheet, r, 1, 1, 7); r++;
  }
  r += 2;
...
//    中途HRMOSシートと同型のシート内完結レポート。既存タブ(211等)・既存V2配信は不変。
//    既存集計(loadRawRecords/aggregate*/_funnelConversion/_landingByPhase/_channelQuality/
//    aggregateBySchool/_monthlyKpiSnapshot/generateOverviewInsights)をそのまま再利用。
// ============================================================

const REPORT_SHEET = "レポート出力";
const REPORT_COLS  = 12;

function _rptPeriodMonth_(year, month) {
  return { year: year, month: month, ym: `${year}-${String(month).padStart(2,"0")}`, label: `${year}年${month}月` };
}
function _rptPrevMonth_() { const n = new Date(); const d = new Date(n.getFullYear(), n.getMonth()-1, 1); return _rptPeriodMonth_(d.getFullYear(), d.getMonth()+1); }
function _rptCurMonth_()  { const n = new Date(); return _rptPeriodMonth_(n.getFullYear(), n.getMonth()+1); }
function _rptFmtR_(x) { return (x === null || x === undefined) ? "-" : Math.round(x*100) + "%"; }

// 週次トレンド履歴（直近8週・ScriptProperties）。週次タブ更新時のみ追記。
function _rptLoadWeeklyHistory_() {
  const raw = PropertiesService.getScriptProperties().getProperty("WEEKLY_HISTORY");
  try { return raw ? JSON.parse(raw) : []; } catch (e) { return []; }
}
function _rptRecordWeeklyHistory_(kpi) {
  let hist = _rptLoadWeeklyHistory_();
  hist.push(Object.assign({ ts: new Date().toISOString() }, kpi));
  if (hist.length > 8) hist = hist.slice(-8);
  PropertiesService.getScriptProperties().setProperty("WEEKLY_HISTORY", JSON.stringify(hist));
  return hist;
}

// 表描画（既存 _tableHeader/_tableData を流用）。合計行は sumRow で強調。
function _rptTable_(sheet, r, header, rows) {
  const cols = header.length;
  sheet.getRange(r, 1, 1, cols).setValues([header]);
  _tableHeader(sheet, r, 1, cols);
  r++;
  if (rows.length) {
    sheet.getRange(r, 1, rows.length, cols).setValues(rows);
    _tableData(sheet, r, 1, rows.length, cols);
    rows.forEach((row, i) => {
      if (String(row[0]).startsWith("合計")) sheet.getRange(r+i, 1, 1, cols).setBackground(COLOR.sumRow).setFontWeight("bold");
    });
    r += rows.length;
  }
  return r;
...
// 注記（複数行・マージ）
function _rptNote_(sheet, r, text) {
  ...
  for (let i = 0; i < n; i++) {
    ...
    sheet.getRange(r+i, 1).setValue(lines[i] || "").setWrap(true).setVerticalAlignment("top")
      .setFontFamily(FONT).setFontSize(10).setFontColor(COLOR.textMain);
  }
  sheet.getRange(r, 1, n, REPORT_COLS).setBorder(true, true, true, true, false, false, COLOR.borderGray, SpreadsheetApp.BorderStyle.SOLID);
  return r + n;
}
...
function buildReportTab_(period) {
  ...
  const targetsCh = loadTargetsChannel();
  const today = new Date();
  const pk = (label) => { const p = phases.find(x => x.label === label); return p ? p.key : null; };

  const sheet = _getOrCreateSheet(REPORT_SHEET);
  _clearSheet(sheet);

  let r = 1;
  _bannerTitle(sheet, r, REPORT_COLS, `■ ${company} 新卒採用レポート`, `基準日 ${formatDate(today)} ／ 生成 ${formatDatetime(today)}`);
  r += 2;
  r = _rptMonthly_(sheet, r, records, phases, targets, targetsCh, period, pk);
  r += 1;
  r = _rptWeekly_(sheet, r, records, phases, targets, targetsCh, today, pk);

  sheet.setColumnWidth(1, 200);
  for (let c = 2; c <= REPORT_COLS; c++) sheet.setColumnWidth(c, 90);
  return sheet;
}

function _rptMonthly_(sheet, r, records, phases, targets, targetsCh, period, pk) {
  _bannerTitle(sheet, r, REPORT_COLS, `■ 月次レポート（経営向け）`, period.label); r += 2;

  // ▎当月KPIサマリ（前月比）
  const cur = _monthlyKpiSnapshot(records, phases, period.year, period.month);
  const pd  = new Date(period.year, period.month - 2, 1);
  const prv = _monthlyKpiSnapshot(records, phases, pd.getFullYear(), pd.getMonth() + 1);
  _sectionHeader(sheet, r, REPORT_COLS, "▎当月KPIサマリ（前月比）"); r++;
  r = _rptTable_(sheet, r, ["指標", "実績", "前月比"], [
    ["新規エントリー", cur.entry,    _diffLabel(cur.entry, prv.entry)],
    ["説明会参加",     cur.briefing, _diffLabel(cur.briefing, prv.briefing)],
    ["一次面接参加",   cur.interview1, _diffLabel(cur.interview1, prv.interview1)],
    ["最終選考参加",   cur.final,    _diffLabel(cur.final, prv.final)],
    ["内定",           cur.offer,    _diffLabel(cur.offer, prv.offer)],
    ["内定承諾",       cur.accept,   _diffLabel(cur.accept, prv.accept)],
  ]); r++;

  // ▎目標 vs 実績
  const tgtForLabel = (label) => { const p = phases.find(x => x.label === label); return p ? (targets[p.key]?.[cur.ym] || 0) : 0; };
  _sectionHeader(sheet, r, REPORT_COLS, "▎目標 vs 実績（当月）"); r++;
  const tgtRows = [
    ["新規エントリー", cur.entry,    "エントリー"],
    ["説明会参加",     cur.briefing, "説明選考会【参加】"],
    ["一次面接参加",   cur.interview1, "一次面接【参加】"],
    ["最終選考参加",   cur.final,    "最終選考【参加】"],
    ["内定",           cur.offer,    "内定"],
  ].map(([disp, act, label]) => { const t = tgtForLabel(label); return [disp, act, t ? t : "-", _targetDiffLabel(act, t)]; });
  r = _rptTable_(sheet, r, ["指標", "実績", "目標", "差分(達成率)"], tgtRows); r++;

  // ▎ファネル通過率 / ボトルネック
  _sectionHeader(sheet, r, REPORT_COLS, "▎ファネル通過率 / ボトルネック（当月・対目標）"); r++;
  const conv = _funnelConversion(records, phases, targets, cur.ym);
  const convRows = conv.steps.filter(s => s.actRate !== null).map(s => {
    const isBn = conv.bottleneck && conv.bottleneck.from === s.from && conv.bottleneck.to === s.to;
    const mark = isBn ? "🔴ボトルネック" : (s.gap !== null && s.gap < -0.05 ? "⚠️" : "");
    return [`${s.from}→${s.to}`, `${s.aFrom}→${s.aTo}`, _rptFmtR_(s.actRate), _rptFmtR_(s.tgtRate), mark];
  });
  r = _rptTable_(sheet, r, ["遷移", "到達数", "実績通過率", "目標通過率", "判定"], convRows.length ? convRows : [["（データ不足）","","","",""]]); r++;
  ...
  try {
    ...
    if (offerRow && offerRow.target > 0) {
      const ratio = offerRow.ratio || 0;
      const judge = ratio >= 1 ? "✅ 達成ペース" : ratio >= 0.8 ? "🟡 やや未達" : "🔴 未達ペース";
      _sectionHeader(sheet, r, REPORT_COLS, "▎着地ペース（内定・予測 vs 期間目標）"); r++;
      r = _rptNote_(sheet, r, `着地予測 ${offerRow.landing.toFixed(1)}名 ／ 期間目標 ${offerRow.target}名 ＝ ${Math.round(ratio*100)}%　${judge}\n※現在選考中の学生を実績通過率で進めた予測`); r++;
    }
  } catch (e) { Logger.log("rpt landing skip: " + e.message); }

  // ▎大学別 歩留まり（主要校トップ8）★深掘り
  _sectionHeader(sheet, r, REPORT_COLS, "▎大学別 歩留まり（エントリー上位8校・累計）"); r++;
  const schoolLabels = ["説明選考会【参加】", "一次面接【参加】", "最終選考【参加】", "内定"];
  const sAgg = aggregateBySchool(records, phases);
  const schoolRows = Object.entries(sAgg)
    .filter(([s]) => s && s !== "（未記入）")
    .map(([s, st]) => [s, st.entry].concat(schoolLabels.map(l => { const k = pk(l); return k ? (st.byPhase[k] || 0) : 0; })))
    .sort((a, b) => b[1] - a[1]).slice(0, 8);
  r = _rptTable_(sheet, r, ["大学", "エントリー", "説明会参加", "一次参加", "最終参加", "内定"], schoolRows.length ? schoolRows : [["（データなし）","","","","",""]]); r++;

  // ▎経路別 質 / CVR ★深掘り
  _sectionHeader(sheet, r, REPORT_COLS, "▎経路別 質 / CVR（内定率順）"); r++;
  const cq = _channelQuality(records, phases).slice(0, 8);
  const cqRows = cq.map(c => [c.name, c.entry, c.pass1, c.offer, _rptFmtR_(c.pass1Rate), _rptFmtR_(c.offerRate)]);
  r = _rptTable_(sheet, r, ["媒体", "エントリー", "一次合格", "内定", "一次合格率", "内定率"], cqRows.length ? cqRows : [["（データなし）","","","","",""]]); r++;

  // ▎月次トレンド（直近6ヶ月）★トレンド
  _sectionHeader(sheet, r, REPORT_COLS, "▎月次トレンド（直近6ヶ月）"); r++;
  const tr = _monthlyTrendData(records, phases, period.year, period.month, 6);
  r = _rptTable_(sheet, r, ["指標"].concat(tr.labels), [
    ["エントリー"].concat(tr.entries),
    ["内定"].concat(tr.offers),
  ]); r++;

  // ▎所感（Good/More＋Gemini一言・維持）
  _sectionHeader(sheet, r, REPORT_COLS, "▎所感"); r++;
  let noteLines = [];
  try {
    const ins = generateOverviewInsights(records, phases, targets, targetsCh);
    const goods = ins.filter(i => i.icon === "✅").slice(0, 3).map(i => `・[Good] ${i.title}: ${i.text}`);
    const mores = ins.filter(i => i.icon === "⚠️" || i.icon === "🔴").slice(0, 3).map(i => `・[課題] ${i.title}: ${i.text}`);
    noteLines = goods.concat(mores);
  } catch (e) { Logger.log("rpt insight skip: " + e.message); }
  try {
    const summary = `当月(${period.label}): エントリー${cur.entry} 説明会${cur.briefing} 一次${cur.interview1} 最終${cur.final} 内定${cur.offer} 承諾${cur.accept}` +
      (conv.bottleneck ? ` / ボトルネック ${conv.bottleneck.from}→${conv.bottleneck.to}` : "");
    const oneLiner = generateGeminiOneLiner(summary);
    if (oneLiner) noteLines.push(`・[一言] ${oneLiner}`);
  } catch (e) { Logger.log("rpt gemini skip: " + e.message); }
  if (!noteLines.length) noteLines = ["（特記事項なし）"];
  r = _rptNote_(sheet, r, noteLines.join("\n")); r++;

  return r;
}

function _rptWeekly_(sheet, r, records, phases, targets, targetsCh, today, pk) {
  _bannerTitle(sheet, r, REPORT_COLS, `■ 週次レポート（担当者向け）`, `〜${formatDate(today)} 基準（直近7日）`); r += 2;

  // ▎今週の動き（直近7日）
  const w = aggregatePast7DaysActuals(records, phases, today);
  _sectionHeader(sheet, r, REPORT_COLS, "▎今週の動き（直近7日・実施日ベース）"); r++;
  r = _rptTable_(sheet, r,
    ["新規エントリー", "説明会参加", "一次参加", "二次参加", "最終参加", "内定"],
    [[w.newEntry, w.briefing, w.interview1, w.interview2, w.interviewF, w.offer]]); r++;

  // ▎現在ステータス（累積・前週比）
  _sectionHeader(sheet, r, REPORT_COLS, "▎現在のステータス（累積・前週比／離脱除く）"); r++;
  const cumActive = aggregateCumulativeActive(records, phases);
  const prev = loadPrevWeekSnapshot();
  const statusOrder = phases.filter(p => p.group === "main" || p.group === "pass" || p.group === "reserved");
  const statusRows = statusOrder.map(p => {
    const cnt = cumActive[p.key] || 0;
    let diff = "—";
    if (prev && prev.counts) { const d = cnt - (prev.counts[p.key] || 0); diff = d === 0 ? "±0" : (d > 0 ? `+${d}` : `${d}`); }
    return [p.label, cnt, diff];
  });
  r = _rptTable_(sheet, r, ["フェーズ", "現在", "前週比"], statusRows); r++;

  // ▎媒体別 新規 / 累計
  _sectionHeader(sheet, r, REPORT_COLS, "▎媒体別 応募数（今週新規 / 累計）"); r++;
  const chb = aggregateChannelBreakdown(records, phases, today);
  const denom = chb.totalEntries || 1;
  const chRows = Object.keys(chb.total).sort((a, b) => chb.total[b] - chb.total[a])
    .map(ch => [ch, chb.week[ch] || 0, chb.total[ch] || 0, (chb.total[ch] / denom * 100).toFixed(1) + "%"]);
  r = _rptTable_(sheet, r, ["媒体", "今週新規", "累計", "構成比"], chRows.length ? chRows : [["（データなし）","","",""]]); r++;

  // ▎大学別 現況（主要校トップ8・累計）★深掘り
  _sectionHeader(sheet, r, REPORT_COLS, "▎大学別 現況（エントリー上位8校）"); r++;
  const sAgg = aggregateBySchool(records, phases);
  const offerKey = pk("内定");
  const schoolRows = Object.entries(sAgg).filter(([s]) => s && s !== "（未記入）")
    .map(([s, st]) => [s, st.entry, offerKey ? (st.byPhase[offerKey] || 0) : 0])
    .sort((a, b) => b[1] - a[1]).slice(0, 8);
  r = _rptTable_(sheet, r, ["大学", "エントリー累計", "内定"], schoolRows.length ? schoolRows : [["（データなし）","",""]]); r++;

  // ▎週次トレンド（直近N週・履歴）★トレンド
  _sectionHeader(sheet, r, REPORT_COLS, "▎週次トレンド（直近の週次更新履歴）"); r++;
  const hist = _rptLoadWeeklyHistory_();
  if (hist.length) {
    const histRows = hist.map(h => [String(h.ts).slice(0, 10), h.newEntry || 0, h.interview1 || 0, h.offer || 0]);
    r = _rptTable_(sheet, r, ["更新日", "新規エントリー", "一次参加", "内定"], histRows);
  } else {
    r = _rptNote_(sheet, r, "（週次トリガー実行ごとに履歴を蓄積します。次回以降の更新で推移が表示されます）");
  }
  r++;

  // ▎要対応（停滞）
  _sectionHeader(sheet, r, REPORT_COLS, "▎要対応（合否未入力・承諾待ち／3日以上）"); r++;
  const actions = extractCriticalActions(records, phases, today, 12);
  const actRows = actions.map(a => [a.urgency, a.type, a.name, a.school || "", `${a.days}日経過`]);
  r = _rptTable_(sheet, r, ["緊急", "種別", "候補者", "大学", "経過"], actRows.length ? actRows : [["—","","","",""]]); r++;
...
function generateReportTabPrevMonth() {
  const p = _rptPrevMonth_();
  const sheet = buildReportTab_(p);
  try { sheet.activate(); SpreadsheetApp.getUi().alert(`レポート出力タブを生成しました（月次: ${p.label}）`); } catch (e) { Logger.log("レポート出力タブ生成: " + p.label); }
}
function generateReportTabForMonth() {
  ...
  const sheet = buildReportTab_(_rptPeriodMonth_(parseInt(m[1],10), parseInt(m[2],10)));
  try { sheet.activate(); } catch (e) {}
}
function buildReportTabAuto()     { buildReportTab_(_rptPrevMonth_()); }     // 月次トリガー: 前月
function refreshWeeklyReportTab() {                                          // 週次トリガー: 当月+週次履歴記録
  const { records, phases } = loadRawRecords();
  try { _rptRecordWeeklyHistory_(aggregatePast7DaysActuals(records, phases, new Date())); } catch (e) { Logger.log("weekly history skip: " + e.message); }
  buildReportTab_(_rptCurMonth_());
}
function _rptDeleteTriggers_(fnName) {
  ScriptApp.getProjectTriggers().forEach(t => { if (t.getHandlerFunction() === fnName) ScriptApp.deleteTrigger(t); });
}
function setReportTabMonthlyTrigger() {
  _rptDeleteTriggers_("buildReportTabAuto");
  ScriptApp.newTrigger("buildReportTabAuto").timeBased().onMonthDay(1).atHour(0).create();
  try { SpreadsheetApp.getUi().alert("月次トリガー設定: 毎月1日0時に前月分レポートタブを自動生成"); } catch (e) {}
}
function setReportTabWeeklyTrigger() {
  _rptDeleteTriggers_("refreshWeeklyReportTab");
  ScriptApp.newTrigger("refreshWeeklyReportTab").timeBased().onWeekDay(ScriptApp.WeekDay.MONDAY).atHour(9).create();
  try { SpreadsheetApp.getUi().alert("週次トリガー設定: 毎週月曜9時にレポートタブ(週次)を自動更新"); } catch (e) {}
}


// ============================================================
// §  月間Doc 追加グラフ（QuickChart・見やすさ強化）
// ============================================================
...
// グループ縦棒（目標=グレー / 実績=朱・データラベル）
function _qcGroupBar_(labels, series, title) {
  return _qcImg_({
    type: "bar",
    data: { labels: labels, datasets: series.map(function (s) { return { label: s.label, data: s.data, backgroundColor: s.color, borderRadius: 4, borderSkipped: false, barPercentage: 0.72, categoryPercentage: 0.62 }; }) },
    options: {
      plugins: {
        title: { display: true, text: title, color: DOC_INK, font: { size: 13, weight: "bold" }, align: "start", padding: { bottom: 10 } },
        legend: { position: "top", align: "end", labels: { color: DOC_INK, usePointStyle: true, boxWidth: 8, font: { size: 11 } } },
        datalabels: { anchor: "end", align: "end", color: DOC_INK, font: { weight: "bold", size: 10 } },
      },
      scales: { x: { grid: { display: false }, border: { display: false }, ticks: { color: DOC_INK, font: { size: 10 } } }, y: { beginAtZero: true, grid: { color: DOC_GRID }, border: { display: false }, ticks: { color: DOC_MUTED, font: { size: 10 } } } },
    },
  }, 720, 320, "groupbar");
}
// 達成率の色分け表（緑≥100% / 黄80-99% / 赤<80%）。rows=[{disp,actual,tgt}]
function _rpoTargetTable_(body, rows) {
  const header = ["指標", "目標", "実績", "達成率"];
  const data = rows.map(function (r) { const rate = r.tgt ? Math.round(r.actual / r.tgt * 100) : null; return [r.disp, r.tgt ? String(r.tgt) : "-", String(r.actual), rate === null ? "-" : rate + "%"]; });
  const t = body.appendTable([header].concat(data));
  try {
    for (let c = 0; c < header.length; c++) { const cell = t.getRow(0).getCell(c); cell.setBackgroundColor(DOC_INK); cell.editAsText().setForegroundColor("#FFFFFF").setBold(true).setFontSize(10); }
    for (let i = 0; i < rows.length; i++) {
      const r = rows[i]; const rate = r.tgt ? (r.actual / r.tgt) : null;
      for (let c = 0; c < header.length; c++) { try { t.getRow(i + 1).getCell(c).editAsText().setFontSize(10); } catch (e) {} }
      const cell = t.getRow(i + 1).getCell(3);
      if (rate !== null) { cell.setBackgroundColor(rate >= 1 ? COLOR.ok : rate >= 0.8 ? COLOR.warn : COLOR.alert); cell.editAsText().setBold(true); }
    }
  } catch (e) {}
  return t;
}

// 月間Docへ ファネル/媒体/大学 グラフを追記（横棒・朱・データラベル）
function _rpoAppendDocCharts_(body, records, phases, ym) {
  const pk = (label) => { const p = phases.find(x => x.label === label); return p ? p.key : null; };
  const fStats = aggregateFunnelMonthlyOwn(records, phases);
  const cnt = (label) => { const k = pk(label); return k ? (fStats[k] && fStats[k][ym] || 0) : 0; };
  // ファネル（合格ベース・当月）= KPIサマリーと一貫。表は付けない（下の「ファネル通過率(対目標)」と重複のため）
  const fLabels = ["エントリー", "説明会合格", "一次合格", "インターン合格", "二次合格", "内定"];
  const fSrc    = ["エントリー", "説明選考会【合格】", "一次面接【合格】", "1dayインターン【合格】", "二次面接【合格】", "内定"];
  const fb = _qcBarH_(fLabels, fSrc.map(cnt), "選考ファネル（当月・合格ベース）", { shade: true });
  if (fb) { _docH_(body, "選考ファネル（歩留まり）"); body.appendImage(fb).setWidth(500).setHeight(250); }
  // 媒体・経路別チャートは「媒体・エージェント別（応募元内訳）」節（単月詳細）に一本化したため、ここでは描画しない（重複解消）。
  // 大学別（累計Top10）— 下の詳細表(Top10)と件数を揃える
  try {
    const sAgg = aggregateBySchool(records, phases);
    const top = Object.entries(sAgg).filter(([s]) => s && s !== "（未記入）").sort((a, b) => b[1].entry - a[1].entry).slice(0, 10);
    if (top.length) { const bb = _qcBarH_(top.map(x => x[0]), top.map(x => x[1].entry), "大学別エントリー（累計Top10）"); if (bb) { _docH_(body, "大学別エントリー（累計Top10）"); body.appendImage(bb).setWidth(540).setHeight(64 + top.length * 30); } }
  } catch (e) { Logger.log("school chart skip: " + e.message); }
}
