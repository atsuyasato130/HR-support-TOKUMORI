...
var MASTER_COLS = [
  ...
  '面接回数', '直近面接日', '直近面接官', '直近フェーズ', '総合評価', '最新評価', '実施内訳'   // ← 11_から自動集計(集計専用)
];
...
var COCKPIT_AUTO_COLS = ['候補者ID', '氏名', '区分', '滞在日数', '停滞', '調整期限', '期限状態', '次回面接日時', '評価未入力', '実施内訳', '優先度'];
...
var COCKPIT_COLS = ['候補者ID', '氏名', '区分', '現ステージ', 'ステータス', '滞在日数', '停滞', '調整期限', '期限状態',
  '次回面接日時', '評価未入力', '実施内訳', '優先度', 'ネクストアクション', 'NA期限', '採用担当RC',
...
var CONF_LAYOUT = {
  ...
  PANEL:  { col: 8,  w: 4, titleRow: 98, row: 100, max: 20, type: 'panel',  // max=PANELS.length(固定)
...
function onOpen() {
  SpreadsheetApp.getUi().createMenu('採用')
    ...
    .addItem('ヨミ会議スナップを確定（MTG終了時）', 'yomiSnapshotSave')
    .addSeparator()
...
function buildMaster_() {
  ...
  var WIDTHS = { 'candidate_id': 110, '区分': 70, '職種': 110, 'チャネル': 100, '氏名': 90, '性別': 64,
    ...
    '面接回数': 64, '直近面接日': 100, '直近面接官': 100, '直近フェーズ': 90, '総合評価': 72, '最新評価': 180, '実施内訳': 170 };
...
function renderPanel_(sh, row, key, M, seg) {
  switch (key) {
    ...
    case 'touchcount': {
      var tc2 = buildTouchCount_(M);
      row = section_(sh, row, '⑳ 接触回数×成果（実施済の面談・面接 総回数と内定/承諾の関係）', tc2.byCount, [4]);
      return section_(sh, row, '　⑳-2 ステージ別 平均実施回数（全体 vs 承諾者 — 承諾者は何回会っているか）', tc2.byStage, [3]);
    }
  }
  return row;
}

// ⑳ 接触回数×成果: 11_の実施済行から (1)総接触回数バケット別の内定/承諾率 (2)ステージ別 平均実施回数(全体vs承諾者)。
// 「承諾に至った学生は何回・どの面談を重ねたか」を見る(カジュアル面談の効き目分析)。
function buildTouchCount_(M) {
  var H = M.H;
  // 11_実施済: 候補者ごとの総回数＋ステージ別回数
  var per = {};  // cid -> { n, stages: {} }
  ...
  if (iv && iv.getLastRow() >= 2) {
    ...
    ivd.forEach(function (r) {
      ...
      var stg = String(r[ivH['ステージ']] || '').trim() || '(不明)';
      if (!per[cid]) per[cid] = { n: 0, stages: {} };
      per[cid].n++; per[cid].stages[stg] = (per[cid].stages[stg] || 0) + 1;
    });
  }
  var iOff = idxByType_(M.stageTypes, ['offer', '合格']);
  function reachedOffer_(r) {
    var st = String(r[H['ステータス']] || '');
    if (st === '内定' || st === '承諾' || st === '入社' || (r[H['内定日']] instanceof Date)) return true;
    var si = M.idxOf[String(r[H['現ステージ']] || '')];
    return (si != null && iOff >= 0 && si >= iOff);
  }
  function accepted_(r) { var st = String(r[H['ステータス']] || ''); return st === '承諾' || st === '入社' || (r[H['承諾日']] instanceof Date); }
  // 表1: 総接触回数バケット別
  var BUCKETS = ['0回', '1回', '2回', '3回', '4回以上'];
  var bk = {}; BUCKETS.forEach(function (b) { bk[b] = { n: 0, off: 0, acc: 0 }; });
  // 表2: ステージ別(全体/承諾者の実施回数)
  var stAgg = {};  // stage -> { total: 実施総数, accTotal: 承諾者の実施総数 }
  var candTotal = 0, accCand = 0;
  M.rows.forEach(function (r) {
    var cid = String(r[H['candidate_id']] || '').trim(); if (!cid) return;
    candTotal++;
    var p = per[cid] || { n: 0, stages: {} };
    var acc = accepted_(r);
    if (acc) accCand++;
    var b = p.n >= 4 ? '4回以上' : (p.n + '回');
    bk[b].n++;
    if (reachedOffer_(r)) bk[b].off++;
    if (acc) bk[b].acc++;
    for (var s in p.stages) {
      if (!stAgg[s]) stAgg[s] = { total: 0, accTotal: 0 };
      stAgg[s].total += p.stages[s];
      if (acc) stAgg[s].accTotal += p.stages[s];
    }
  });
  var t1 = BUCKETS.filter(function (b) { return bk[b].n; }).map(function (b) {
    var x = bk[b]; return [b, x.n, x.off, x.acc, pct_(x.acc, x.n)];
  });
  // ステージ表示順=区分のステージ定義順・未知は末尾
  var order = M.stageNames.slice();
  Object.keys(stAgg).forEach(function (s) { if (order.indexOf(s) < 0) order.push(s); });
  function avg_(sum, n) { return n > 0 ? String(Math.round(sum / n * 10) / 10) : '—'; }
  var t2 = order.filter(function (s) { return stAgg[s]; }).map(function (s) {
    var x = stAgg[s];
    return [s, x.total, avg_(x.total, candTotal), avg_(x.accTotal, accCand)];
  });
  return {
    byCount: { header: ['総接触回数', '人数', '内定到達', '承諾', '承諾率'], rows: t1.length ? t1 : [['(実施データなし)', 0, 0, 0, '—']] },
    byStage: { header: ['ステージ', '実施総数', '全体平均(回/人)', '承諾者平均(回/人)'], rows: t2.length ? t2 : [['(実施データなし)', 0, '—', '—']] }
  };
}
...
// MTGで使う会議ボード: KPI帯(着地見込vs目標)＋前回スナップ比の変化＋討議順ソート＋MTG決定事項/宿題。
var YOMI_SUFFIX = '_ヨミ管理';
// ③新卒_ヨミ管理: 読取(マスタ自動)列 + ③保持(入力)列。新卒以外はヨミを②に内包(segOwnExtra_)。
var YOMI_READ = ['候補者ID', '氏名', '大学', '学部', '学科', '性別', '職種', 'チャネル', '採用担当RC', '現ステージ', 'ステータス', '総合評価', '実施内訳', '直近面接日', '内定日', '承諾日'];
var YOMI_OWN3 = YOMI_OWN.concat(['ネクストアクション', 'MTG決定事項', '宿題(担当と内容)', '宿題期限', 'メモ']);  // ③はマスタ非同期=③ローカル保持(keep)
var YOMI_SURVEY = ['アンケート満足度', '志望度'];  // ②進捗管理(新卒)から候補者IDで読む(③でアンケート結果も確認・読取専用)
// 期待値の重み(着地見込 = 承諾済×1.0 + Σランク重み)。運用で調整したくなったら01_設定へ昇格する。
var YOMI_WEIGHTS = { 'S': 0.9, 'A': 0.7, 'B': 0.5, 'C': 0.2 };
function yomiName_(seg) { return (seg + YOMI_SUFFIX).slice(0, 95); }
function yomiSnapKey_(seg) { return 'yomiSnap_' + seg; }
// ヨミ会議スナップを保存(MTG終了時にメニューから実行)。次回のMTGで「前回比」がこの時点との差分になる。
function yomiSnapshotSave() {
  var saved = [];
  enabledSegments_().forEach(function (seg) {
    var sh = ss_().getSheetByName(yomiName_(seg)); if (!sh || sh.getLastRow() < 3) return;
    var lc = sh.getLastColumn();
    var head = sh.getRange(2, 1, Math.min(3, sh.getLastRow() - 1), lc).getValues();
    var hRow = -1, oH = {};
    for (var hi = 0; hi < head.length; hi++) { if (head[hi].indexOf('候補者ID') >= 0) { hRow = 2 + hi; head[hi].forEach(function (h, i) { oH[String(h)] = i; }); break; } }
    if (hRow < 0 || oH['候補者ID'] == null) return;
    var last = sh.getLastRow(); if (last <= hRow) return;
    var d = sh.getRange(hRow + 1, 1, last - hRow, lc).getValues(), map = {};
    d.forEach(function (r) {
      var c = String(r[oH['候補者ID']] || '').trim(); if (!c) return;
      map[c] = { r: String(oH['ヨミランク'] != null ? r[oH['ヨミランク']] : '').charAt(0),
                 s: String(oH['現ステージ'] != null ? r[oH['現ステージ']] : '').trim(),
                 st: String(oH['ステータス'] != null ? r[oH['ステータス']] : '').trim() };
    });
    PropertiesService.getDocumentProperties().setProperty(yomiSnapKey_(seg),
      JSON.stringify({ ts: Utilities.formatDate(new Date(), ss_().getSpreadsheetTimeZone(), 'MM/dd HH:mm'), map: map }));
    try { buildYomi_(seg); } catch (e) { Logger.log('yomiSnapshotSave rebuild: ' + e); }
    saved.push(seg);
  });
  toast_(saved.length ? ('ヨミ会議スナップを確定しました（' + saved.join('/') + '）。次回MTGの「前回比」はこの時点との差分になります') : 'ヨミ管理タブが見つかりません');
}
// ヨミ母集団(クロージング対象 or 内定/承諾/入社・見送り辞退は除外)。buildYomi_ とヨミ計測パネルで共有。
// 閾値=内定(offer)ステージの2つ手前から(例: …二次面接→最終面接→内定 なら二次面接以降)。offer段が無い区分は終盤3ステージ。
function yomiPopulation_(M) {
  var H = M.H, n = M.stageNames.length;
  var iOff = idxByType_(M.stageTypes, ['offer', '合格']);
  var threshold = (iOff >= 0) ? Math.max(0, iOff - 2) : Math.max(0, n - 3);
...
function buildYomi_(seg) {
  ...
  var cols = ['前回比'].concat(YOMI_READ).concat(YOMI_SURVEY).concat(YOMI_OWN3), w = cols.length;
  // keep退避: ヘッダー行を動的検出(旧レイアウト=行2 / 会議ボード=行3)して OWN列を候補者IDで退避
  var keep = {};
  if (existed && sh.getLastRow() >= 3 && sh.getLastColumn() >= 3) {
    var od = sh.getRange(2, 2, sh.getLastRow() - 1, sh.getLastColumn() - 1).getValues();
    var hIdx = -1;
    for (var hi = 0; hi < Math.min(2, od.length); hi++) { if (od[hi].indexOf('候補者ID') >= 0) { hIdx = hi; break; } }
    if (hIdx >= 0) {
      var oH = {}; od[hIdx].forEach(function (h, i) { oH[String(h)] = i; });
      for (var i = hIdx + 1; i < od.length; i++) { var c0 = String((oH['候補者ID'] != null ? od[i][oH['候補者ID']] : '') || '').trim(); if (!c0) continue; keep[c0] = {}; YOMI_OWN3.forEach(function (cn) { if (oH[cn] != null) keep[c0][cn] = od[i][oH[cn]]; }); }
    }
  }
  // 前回スナップ(MTG終了時に確定)との差分
  var snap = null;
  try { snap = JSON.parse(PropertiesService.getDocumentProperties().getProperty(yomiSnapKey_(seg)) || 'null'); } catch (e) { snap = null; }
  var rankIdx = { 'S': 0, 'A': 1, 'B': 2, 'C': 3 };
  function isSettled_(st) { return st === '承諾' || st === '入社'; }
  function diffOf_(cid, rank, stageName, status) {
    if (!snap || !snap.map) return '';
    var p = snap.map[cid];
    if (!p) return 'NEW';
    if (isSettled_(status) && !isSettled_(p.st)) return '承諾◎';
    if (status === '辞退' || status === '見送り') return '';  // 母集団から消えるので通常出ない
    var ri0 = (p.r && rankIdx[p.r] != null) ? rankIdx[p.r] : 99;
    var ri1 = (rank && rankIdx[rank] != null) ? rankIdx[rank] : 99;
    if (ri1 < ri0) return '↑' + (p.r || '未') + '→' + rank;
    if (ri1 > ri0) return '↓' + (p.r || '未') + '→' + (rank || '未');
    var si0 = M.idxOf[String(p.s || '')], si1 = M.idxOf[String(stageName || '')];
    if (si0 != null && si1 != null && si1 > si0) return '→進行';
    return '';
  }
  // 討議順の下ごしらえ: 決着(承諾/入社)は最下部 → ヨミランクS→A→B→C→未設定 → 同ランク内はステージ降順
  var items = hot.map(function (r) {
    var cid = String(r[H['candidate_id']] || ''), kp = keep[cid] || {};
    var status = String(r[H['ステータス']] || '').trim();
    var rank = String(kp['ヨミランク'] || '').charAt(0);
    var stageName = String(r[H['現ステージ']] || '').trim();
    return { r: r, cid: cid, kp: kp, status: status, rank: rank, stage: stageName,
             si: (M.idxOf[stageName] != null) ? M.idxOf[stageName] : -1,
             settled: isSettled_(status), diff: diffOf_(cid, rank, stageName, status) };
  });
  items.sort(function (a, b) {
    if (a.settled !== b.settled) return a.settled ? 1 : -1;
    var ra = (a.rank && rankIdx[a.rank] != null) ? rankIdx[a.rank] : 9;
    var rb = (b.rank && rankIdx[b.rank] != null) ? rankIdx[b.rank] : 9;
    return ra - rb || b.si - a.si;
  });
  var body = items.map(function (x) {
    return cols.map(function (cn) {
      if (cn === '前回比') return x.diff;
      if (cn === '候補者ID') return x.cid;
      if (YOMI_OWN3.indexOf(cn) >= 0) return (x.kp[cn] != null ? x.kp[cn] : '');
      if (YOMI_SURVEY.indexOf(cn) >= 0) { var sv = surveyMap[x.cid] || {}; return (sv[cn] != null ? sv[cn] : ''); }
      var v = (H[cn] != null) ? x.r[H[cn]] : ''; return (v == null) ? '' : v;
    });
  });
  // KPI帯(着地見込 vs 承諾目標)
  var kpi = { total: items.length, S: 0, A: 0, B: 0, C: 0, unset: 0, settled: 0, exp: 0 };
  items.forEach(function (x) {
    if (x.settled) { kpi.settled++; kpi.exp += 1; return; }
    if (x.rank && YOMI_WEIGHTS[x.rank] != null) { kpi[x.rank]++; kpi.exp += YOMI_WEIGHTS[x.rank]; }
    else kpi.unset++;
  });
  var tgtRow = confRead_('TARGET').filter(function (r) { return String(r[0]) === seg; })[0] || [];
  var accTarget = Number(tgtRow[2] || 0);  // 承諾目標
  var expR = Math.round(kpi.exp * 10) / 10;
  var gapTxt = accTarget > 0 ? ('（差分 ' + ((expR - accTarget >= 0 ? '+' : '') + (Math.round((expR - accTarget) * 10) / 10)) + '名）') : '';
  var kpiText = 'ヨミ対象 ' + kpi.total + '名 ｜ S:' + kpi.S + ' A:' + kpi.A + ' B:' + kpi.B + ' C:' + kpi.C + ' 未設定:' + kpi.unset
    + ' ｜ 着地見込 ' + expR + '名（承諾済' + kpi.settled + '＋期待値・重み S90% A70% B50% C20%）'
    + (accTarget > 0 ? ' vs 承諾目標 ' + accTarget + '名' + gapTxt : ' ｜ 承諾目標未設定(01_設定❸)')
    + ' ｜ 前回スナップ: ' + (snap && snap.ts ? snap.ts : '未確定（MTG終了時にメニュー「ヨミ会議スナップを確定」）');

  clearSheet_(sh);
  var N = Math.max(items.length + 20, 40);
  var headRow = 3, dataTop = 4;  // 行1=タイトル / 行2=KPI帯 / 行3=列ヘッダー / 行4〜=データ
  ensureGrid_(sh, N + dataTop, w + 1);
  bandTitle_(sh, 1, 2, w + 1, '■ ' + companyName_() + '　' + seg + ' ヨミ会議ボード（採用確度・クロージング・MTG進行用）');
  sh.getRange(2, 2, 1, w).merge().setValue(kpiText)
    .setBackground('#FBF3F1').setFontColor(C.INK).setFontWeight('bold').setFontSize(10)
    .setWrap(true).setVerticalAlignment('middle')
    .setBorder(true, true, true, true, false, false, C.BORDER, SpreadsheetApp.BorderStyle.SOLID);
  sh.setRowHeight(2, 36);
  sh.getRange(headRow, 2, 1, w).setValues([cols]).setBackground(HEAD_BG).setFontColor(C.INK).setFontWeight('bold').setFontSize(9).setHorizontalAlignment('center');
  ...
  var bgs = []; for (var z = 0; z < N; z++) { var line = []; for (var x = 0; x < w; x++) line.push(z % 2 ? C.ZEBRA : C.WHITE); bgs.push(line); }
  sh.getRange(dataTop, 2, N, w).setBackgrounds(bgs);
  YOMI_OWN3.forEach(function (cn) { var i = cols.indexOf(cn); if (i >= 0) sh.getRange(dataTop, 2 + i, N, 1).setBackground(DESIGN_INPUT_BG); });
  ['前回比'].concat(YOMI_READ).concat(YOMI_SURVEY).forEach(function (cn) { var i = cols.indexOf(cn); if (i >= 0) sh.getRange(dataTop, 2 + i, N, 1).setBackground(DESIGN_AUTO_BG); });
  ['直近面接日', '内定日', '承諾日', '宿題期限'].forEach(function (cn) { var i = cols.indexOf(cn); if (i >= 0) sh.getRange(dataTop, 2 + i, N, 1).setNumberFormat('yyyy/mm/dd'); });
  var ri = cols.indexOf('ヨミランク'); if (ri >= 0) sh.getRange(dataTop, 2 + ri, N, 1).setDataValidation(SpreadsheetApp.newDataValidation().requireValueInList(YOMI_RANKS, true).setAllowInvalid(true).build());
  // 条件付き書式: 前回比(NEW=青/↑=緑/↓=赤/承諾◎=緑太字)・宿題期限(期限切れ=赤)
  var rules = [];
  var di = cols.indexOf('前回比');
  if (di >= 0) {
    var rDiff = sh.getRange(dataTop, 2 + di, N, 1);
    rules.push(SpreadsheetApp.newConditionalFormatRule().whenTextEqualTo('NEW').setBackground('#E8F0FE').setFontColor('#1A56DB').setBold(true).setRanges([rDiff]).build());
    rules.push(SpreadsheetApp.newConditionalFormatRule().whenTextStartsWith('↑').setBackground('#E6F4EA').setFontColor('#1E6B2E').setBold(true).setRanges([rDiff]).build());
    rules.push(SpreadsheetApp.newConditionalFormatRule().whenTextStartsWith('↓').setBackground('#F8D7DA').setFontColor(C.RED).setBold(true).setRanges([rDiff]).build());
    rules.push(SpreadsheetApp.newConditionalFormatRule().whenTextStartsWith('承諾').setBackground('#E6F4EA').setFontColor('#1E6B2E').setBold(true).setRanges([rDiff]).build());
  }
  var hi2 = cols.indexOf('宿題期限');
  if (hi2 >= 0) {
    var colL = colLetter_(2 + hi2);
    rules.push(SpreadsheetApp.newConditionalFormatRule()
      .whenFormulaSatisfied('=AND($' + colL + dataTop + '<>"", $' + colL + dataTop + '<TODAY())')
      .setBackground('#F8D7DA').setFontColor(C.RED).setBold(true)
      .setRanges([sh.getRange(dataTop, 2 + hi2, N, 1)]).build());
  }
  sh.setConditionalFormatRules(rules);
  sh.getRange(headRow, 2, N + 1, w).setBorder(true, true, true, true, true, true, C.BORDER, SpreadsheetApp.BorderStyle.SOLID);
  sh.setColumnWidth(1, 28);
  var YW = { '前回比': 90, '実施内訳': 150, 'MTG決定事項': 200, '宿題(担当と内容)': 200, '宿題期限': 90 };
  for (var ci = 0; ci < w; ci++) sh.setColumnWidth(2 + ci, YW[cols[ci]] || manageColWidth_(cols[ci]));
  try { sh.setFrozenRows(headRow); sh.setFrozenColumns(3); } catch (e2) {}
  note_(sh, dataTop + N + 1, w, seg + 'のクロージング対象（内定ステージの2つ手前=終盤選考に到達 or 内定/承諾/入社）を自動抽出した「ヨミ会議ボード」。討議順=ヨミランクS→C(未設定は後・承諾/入社は最下部)。前回比=前回スナップとの変化(NEW=新規入り/↑↓=ランク変動/→進行=ステージ前進/承諾◎=決着)。グレー=自動・ピンク=入力(このタブに保存・再生成でも保持)。MTGの終わりにメニュー「採用 ▸ ヨミ会議スナップを確定」を押すと、次回MTGの前回比がこの時点基準になります。');
...
// ヘッダー行は動的検出(②=行2 / ヨミ会議ボード=行3=KPI帯の下)。
function yomiRankMap_(seg) {
  ...
  var hRow = -1, oH = {};
  for (var hi = 0; hi < head.length; hi++) { if (head[hi].indexOf('候補者ID') >= 0) { hRow = 2 + hi; head[hi].forEach(function (h, i) { oH[String(h)] = i; }); break; } }
  if (hRow < 0 || oH['候補者ID'] == null || oH['ヨミランク'] == null) return {};
  var last = sh.getLastRow(); if (last <= hRow) return {};
  var d = sh.getRange(hRow + 1, 1, last - hRow, lc).getValues(), map = {};
...
function buildYomiMetricsPanel_(sh, row, M, seg) {
  ...
  var stageCount = {}, offers = 0, accepts = 0, expected = 0;
  pop.forEach(function (r) {
    ...
    var settled = (r[H['承諾日']] instanceof Date) || st === '承諾' || st === '入社';
    if (settled) accepts++;
    expected += settled ? 1 : ((rk && YOMI_WEIGHTS[rk] != null) ? YOMI_WEIGHTS[rk] : 0);  // 着地見込=承諾済1.0+ランク重み
  });
  var tgtRow0 = confRead_('TARGET').filter(function (r) { return String(r[0]) === seg; })[0] || [];
  var accTgt0 = Number(tgtRow0[2] || 0), expR0 = Math.round(expected * 10) / 10;
  sh.getRange(row, 2).setValue('▎⑯ ヨミ計測（採用確度・歩留まり・着地見込）').setFontColor(C.BLACK).setFontWeight('bold').setFontSize(11); row += 1;
  row = table_(sh, row, 2, ['指標', '人数/率'], [
    ...
    ['着地見込(承諾済+重み付き期待値)', expR0],
    ['承諾目標', accTgt0 || '—'],
    ['目標差分', accTgt0 ? ((expR0 - accTgt0 >= 0 ? '+' : '') + (Math.round((expR0 - accTgt0) * 10) / 10)) : '—']
  ], [1]);
...
// 11_の「実施済」面接から候補者ごとの 面接回数/直近面接日/直近フェーズ/平均評点/最新評価/実施内訳 をマスタへ反映
function syncInterviewSummary_() {
  ...
  if (ivData.length >= 2) {
    ...
    ivData.forEach(function (r) {
      ...
      if (!agg[cid]) agg[cid] = { count: 0, last: null, lastWho: '', lastStage: '', lastLetter: '', lastMemo: '', stages: {} };
      var a = agg[cid]; a.count++;
      if (stage) a.stages[stage] = (a.stages[stage] || 0) + 1;  // ステージ別実施回数(カジュアル面談N回 等)
      if (dt instanceof Date && (!a.last || dt > a.last)) { a.last = dt; a.lastWho = who; a.lastStage = stage; a.lastLetter = letter; a.lastMemo = memo; }
    });
  }
  // 実施内訳の表示順=ステージ定義順(全有効区分の連結)・未知ステージは末尾
  var stageOrder = {};
  try {
    var so = 0;
    enabledSegments_().forEach(function (s) { segStages_(s).forEach(function (st) { if (stageOrder[st.name] == null) stageOrder[st.name] = so++; }); });
  } catch (e) {}
  function breakdown_(stages) {
    var keys = Object.keys(stages);
    if (!keys.length) return '';
    keys.sort(function (a, b) {
      var oa = (stageOrder[a] != null) ? stageOrder[a] : 999, ob = (stageOrder[b] != null) ? stageOrder[b] : 999;
      return oa - ob || (a < b ? -1 : 1);
    });
    return keys.map(function (k) { return k + stages[k]; }).join('・');
  }
  var mData = m.getDataRange().getValues();
  if (mData.length < 2) return;
  var mH = headerIndex_(mData[0]);
  var hasBd = (mH['実施内訳'] != null);  // 旧ヘッダー(列未追加)のシートでは6列書込にフォールバック
  var out = [];
  for (var i = 1; i < mData.length; i++) {
    var a = agg[String(mData[i][mH['candidate_id']]).trim()];
    var row = a ? [a.count, a.last || '', a.lastWho, a.lastStage, a.lastLetter, a.lastMemo, breakdown_(a.stages)]
                : [0, '', '', '', '', '', ''];
    out.push(hasBd ? row : row.slice(0, 6));
  }
  // 面接回数/直近面接日/直近面接官/直近フェーズ/総合評価/最新評価(/実施内訳) は連続列
  m.getRange(2, mH['面接回数'] + 1, out.length, hasBd ? 7 : 6).setValues(out);
}
...
function buildEvalNote_() {
  ...
  sh.getRange(34, 2).setValue('▎評価履歴（実施済・新しい順・直近30件）').setFontColor(C.BLACK).setFontWeight('bold').setFontSize(11);
  hist = hist.slice(0, 30);  // 全件表示は縦に伸びすぎるため直近30件(全量は11_面接スケジュールが正本)
  var hTop = 35, tz2 = ss_().getSpreadsheetTimeZone();
  // 所感は I:K(3列マージ・計330px)に載せて折返し表示(旧: I列50px単独で潰れていた)。60字超はセルメモに全文。
  sh.getRange(hTop, 2, 1, 7).setValues([['面接日', '候補者', '区分', 'フェーズ', '面接官', '総合', '判定']])
    .setBackground(HEAD_BG).setFontColor(C.INK).setFontWeight('bold').setFontSize(10).setHorizontalAlignment('center');
  var hh = sh.getRange(hTop, 9, 1, 3); hh.breakApart(); hh.merge().setValue('所感')
    .setBackground(HEAD_BG).setFontColor(C.INK).setFontWeight('bold').setFontSize(10).setHorizontalAlignment('center');
  sh.setRowHeight(hTop, 24);
  if (!hist.length) {
    sh.getRange(hTop + 1, 2).setValue('（実施済の評価がまだありません）').setFontColor(C.SUB).setFontSize(10);
  } else {
    hist.forEach(function (r, i) {
      var rr = hTop + 1 + i, zebra = (i % 2 === 1) ? C.ZEBRA : C.WHITE;
      sh.getRange(rr, 2, 1, 10).setBackground(zebra);
      sh.getRange(rr, 2, 1, 7).setValues([[r[0] ? Utilities.formatDate(r[0], tz2, 'MM/dd') : '', r[1], r[2], r[3], r[4], r[5], r[6]]])
        .setFontColor(C.INK).setFontSize(10).setVerticalAlignment('middle');
      sh.getRange(rr, 7, 1, 2).setFontColor(C.RED).setFontWeight('bold');  // 総合(G)/判定(H)
      var memo = String(r[7] || '').replace(/\n/g, ' / ');
      var summary = memo.length > 60 ? memo.slice(0, 60) + '…' : memo;
      var g = sh.getRange(rr, 9, 1, 3); g.breakApart(); g.merge().setValue(summary)
        .setFontColor(C.INK).setFontSize(9).setWrap(true).setVerticalAlignment('middle');
      if (memo.length > 60) g.setNote(memo);  // 全文はセルメモ(クリックで閲覧)
      sh.setRowHeight(rr, 26);
    });
  }
  sh.getRange(hTop, 2, Math.max(hist.length, 1) + 1, 10).setBorder(true, true, true, true, true, true, C.BORDER, SpreadsheetApp.BorderStyle.SOLID);
}

// 申し送り: 指定候補者の過去「実施済」評価を 12_ の申し送り欄(行26〜)に描画
function refreshHandover_(cid) {
  var sh = sheet_(SH.NOTE);
  var area = sh.getRange(26, 2, 8, 10); area.breakApart(); area.clearContent().clearNote().setBackground(C.WHITE);
  // 所感(要約)は G:K(5列マージ)に載せて折返し表示(旧: G列=幅16pxのスペーサーに落ちて潰れていた)。
  function renderHandover_(rows) {
    sh.getRange(26, 2, 1, 5).setValues([['面接日', 'フェーズ', '面接官', '総合', '判定']])
      .setBackground(HEAD_BG).setFontColor(C.INK).setFontWeight('bold').setFontSize(10).setHorizontalAlignment('center');
    var gh = sh.getRange(26, 7, 1, 5); gh.merge().setValue('所感(要約)')
      .setBackground(HEAD_BG).setFontColor(C.INK).setFontWeight('bold').setFontSize(10).setHorizontalAlignment('center');
    sh.setRowHeight(26, 24);
    rows.forEach(function (r, i) {
      var rr = 27 + i, zebra = (i % 2 === 1) ? C.ZEBRA : C.WHITE;
      sh.getRange(rr, 2, 1, 10).setBackground(zebra);
      sh.getRange(rr, 2, 1, 5).setValues([r.slice(0, 5)]).setFontColor(C.INK).setFontSize(10).setVerticalAlignment('middle');
      sh.getRange(rr, 5, 1, 2).setFontColor(C.RED).setFontWeight('bold');  // 総合(E)/判定(F)
      var g = sh.getRange(rr, 7, 1, 5); g.merge().setValue(r[5])
        .setFontColor(C.INK).setFontSize(9).setWrap(true).setVerticalAlignment('middle');
      if (r[6]) g.setNote(r[6]);  // 60字超の全文はセルメモ
      sh.setRowHeight(rr, 28);
    });
    sh.getRange(26, 2, rows.length + 1, 10).setBorder(true, true, true, true, true, true, C.BORDER, SpreadsheetApp.BorderStyle.SOLID);
  }
  if (!cid) { renderHandover_([['候補者IDを入力すると表示されます', '', '', '', '', '', '']]); return; }
  var iv = sheet_(SH.IV); var data = iv.getDataRange().getValues();
  if (data.length < 2) { renderHandover_([['(履歴なし)', '', '', '', '', '', '']]); return; }
  var H = headerIndex_(data.shift()); var list = [];
  data.forEach(function (r) {
    if (String(r[H['candidate_id']]).trim() !== cid) return;
    if (String(r[H['ステータス']]) !== '実施済') return;
    var dt = r[H['予定日時']];
    var full = String(r[H['評価メモ']] || '').replace(/\n/g, ' / ');
    var memo = full.length > 60 ? full.slice(0, 60) + '…' : full;
    list.push([dt instanceof Date ? dt : '', String(r[H['ステージ']] || ''), String(r[H['面接官']] || ''), String(r[H['評点']] || ''), String(r[H['判定']] || ''), memo, full.length > 60 ? full : '']);
  });
  list.sort(function (a, b) { return (a[0] instanceof Date ? a[0].getTime() : 0) - (b[0] instanceof Date ? b[0].getTime() : 0); });
  list = list.slice(0, 6);
  var rows = list.length ? list.map(function (r) { return [r[0] ? Utilities.formatDate(r[0], ss_().getSpreadsheetTimeZone(), 'MM/dd') : '', r[1], r[2], r[3], r[4], r[5], r[6]]; })
    : [['(この候補者の実施済評価はまだありません)', '', '', '', '', '', '']];
  renderHandover_(rows);
}
...
function cockpitData_() {
  ...
  // 区分ごとの最初のステージ名(応募等)。ここに留まり かつ NA/次回面接が無い候補者=未着手プールはコックピットに載せない。
  var firstStage = {};
  try { enabledSegments_().forEach(function (s) { var st = segStages_(s); if (st.length) firstStage[s] = st[0].name; }); } catch (e) {}
  var list = [];
  am.rows.forEach(function (r) {
    var status = String(r[H['ステータス']] || '').trim();
    ...
    // 母集団絞り込み: 最初のステージのままで NA・NA期限・次回面接がすべて空=まだ選考を動かしていない応募者は除外
    // (エントリー母数の管理はダッシュ/マスタの仕事。アンケート自動昇格ONなら説明会参加で自動的にここへ載る)
    if (firstStage[seg] && curStage === firstStage[seg]) {
      var naV = String(r[H['ネクストアクション']] || '').trim();
      var naD = r[H['NA期限']], nx = r[H['次回面接日時']];
      if (!naV && !(naD instanceof Date) && !(nx instanceof Date)) return;
    }
    ...
    list.push({
      ...
      breakdown: (H['実施内訳'] != null) ? String(r[H['実施内訳']] || '') : '',
...
function buildCockpit_() {
  ...
  var CW = { '候補者ID': 100, '氏名': 96, '区分': 60, '現ステージ': 100, 'ステータス': 70, '滞在日数': 64, '停滞': 64,
    '調整期限': 90, '期限状態': 76, '次回面接日時': 120, '評価未入力': 110, '実施内訳': 130, '優先度': 56, 'ネクストアクション': 200,
  ...
  note_(sh, dataTop + N + 1, w, '進行中/内定の候補者を優先度順に表示（優先度=調整期限超過＞NA超過＞評価未入力＞本日＞停滞）。母集団=選考を動かしている候補者（最初のステージのままで NA・NA期限・次回面接が全て空の未着手応募者は表示しない＝エントリー母数はダッシュボードで管理）。グレー=自動。ピンクのうち「' + COCKPIT_SYNC_COLS.join('・') + '」はマスタへ自動同期＝正本はマスタ。面接調整=「確定ステージ・確定日時（必要ならURL/面接官/形式）」を入れて通知送信に☑ → 確定/変更を自動判定し11_面接スケジュールへ即反映＋リマインド再設定。⚠️通知メールの実送信は権限の都合で☑では出ないことがあるため、メニュー「採用 ▸ 面接を確定＆通知」でまとめて送るのが確実（☑分は「要送信」として残ります）。' + (MAIL_MASTER_ENABLED ? '' : '（🔒現在は送信ロック中＝シート反映のみ・メールは1通も送信されません）'));
  return body.length;
}
